...
- **A store's `update(id, changes)` is a full-record `PUT` for most entities, not a partial `PATCH`** — the backend overwrites every mapped field unconditionally, so a field `changes` omits doesn't stay unchanged, it gets zeroed out server-side. Always resend the full current record (spread it, override just the changed field) unless you've confirmed that specific store/handler pair does a real partial merge (e.g. `stores/contracts.ts`'s `update()`, or a store with its own narrow-PATCH action like `stores/deals.ts`'s `updateStage()`) — see `biz_spec/design-system.md` §8 for the full writeup and the 2026-09-15 Prospect/Lead Kanban-drag bug this caused.
