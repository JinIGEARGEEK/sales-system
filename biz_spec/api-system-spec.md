...
| `GET` | `/reports/prospect-source-conversion?assigned_to=&date_from=&date_to=` | **Added 2026-09-03.** `FR-CRM-107`. Conversion rate by `Prospect.source` — exact structural mirror of the row above, one funnel stage earlier: "converted" = `status = 'Converted'` (the same terminal state `Prospect.converted_lead_id` being non-null implies). **Admin/Marketing/Sales Manager** — its own `RequireRoles` group, separate from the Admin/Sales-Manager-only `reports` group every other row on this page lives under, since Marketing has no access to any Deal/Lead-derived report but does own this one. `GET .../export` streams the same rows as CSV, matching every other report's export pattern. |
