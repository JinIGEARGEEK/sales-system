...
| `PATCH` | `/deals/:id/stage` | 🟢 | Body: `{ stage: DealStage }`. Dedicated endpoint for the Kanban drag-and-drop (`CrmPipelineBoard`'s `@move`) so the backend can also update `status` (open/won/lost) in one transaction. **Does not** fire an auto Customer-Product creation on `Won` — `FR-CRM-064` was confirmed 2026-08-22 as intentionally not built (see §8.2 below); CustomerProduct creation stays a manual, client-triggered action (FR-CRM-065) regardless of stage transitions. |
...
### 8.7a Admin app settings (`FR-CRM-058`, `FR-CRM-091`)
