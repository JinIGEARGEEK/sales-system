...
interface Lead {
  ...
  // Added 2026-09-22 (FR-CRM-122) — orders this Lead's Kanban card within its
  // own status lane only, never compared across lanes. Server-computed: see
  // Deal.position's doc comment (§7.1) for the full scheme; set here via
  // PATCH /leads/:id/status below.
  position: number
  // Added 2026-09-23 (FR-CRM-123) — when this Lead entered its current
  // status lane. Server-set on create and on every real status change
  // (PUT/PATCH status/convert); a same-lane reorder leaves it alone.
  stage_entered_at: string | null
  // Added 2026-09-24 — the lane this record left on its last stage/status
  // change (set with stage_entered_at); null if it has never moved.
...
|---|---|---|---|
| `GET` | `/leads` | 🟢 | Filters: `status`, `source`, `assigned_to`, `company_id` (exact match, added 2026-08-24), `search` (name/email/**company name**, via a join now that `company_name` is a real FK — preserves what `search` could already match before the 2026-08-24 migration off free-text `company_name`, unlike Deal/Contact's own `search`, which was never company-name-aware to begin with), `exclude_converted=true` (returns only Leads with `converted_deal_id IS NULL` — used by the Deals Pipeline board, see §7.1, so already-converted Leads don't show up as cards alongside their own resulting Deal), `only_converted=true` (added 2026-09-15, the mirror image: returns only Leads with `converted_deal_id IS NOT NULL` — backs the Leads list's "Converted" scope tab in `pages/crm/leads/index.vue`, see the Status Badge Guideline's Deal-outcome-caption note in `ux-ui-guidelines/table.md` §9). `sort=company_name`/`-company_name` also joins to `companies`, same pattern as Deal/Contact's existing company-name sort special-case (§7.1/§3 elsewhere) — the two joins are shared/deduplicated, not applied twice. A Lead with no `company_id` at all still matches on `name`/`email` and still appears in an otherwise-unfiltered list (`LEFT JOIN`, not `JOIN`). **Updated 2026-09-01**: this nullable-company-id join+search logic is now a shared pair of helpers, `utils.ApplyNullableCompanySearch`/`ApplyNullableCompanySort`, used by both this endpoint and `GET /prospects` below (previously duplicated inline in `LeadHandler.List` only, back when Lead was the only resource with a nullable `company_id` needing it). Otherwise `sort` accepts `created_at` (default, descending) / `name` / **`position`** (added 2026-09-22, FR-CRM-122 — used by the Kanban board so a within-lane reorder's order survives a refetch). Backs `pages/crm/leads/index.vue`. |
| `POST` | `/leads` | 🟢 | Create. `company_id`, if supplied, is optional (unlike Deal/Contact, which require it — though notably, Deal/Contact's own `company_id` is only presence-checked, never existence-checked against the DB either). **Updated 2026-09-16**: Lead's optional `company_id` now *is* existence-checked (`404` if the row doesn't exist) — `validateLeadCompanyID` in `internal/handlers/leads.go`, mirroring `projects.go`'s own Company lookup (the one pre-existing real "does this FK exist" check in this codebase). If the request body omits `assigned_to` (or sends `null`), the backend auto-assigns the new Lead via `pickAutoAssignee()` (`internal/handlers/leads.go`) — a least-open-load strategy that picks whichever active Sales Rep currently has the fewest open Leads/Deals, not a literal round-robin index. This only fires on creation with no explicit assignee; bulk-reassign and Kanban-drag reassignment flows are untouched. `score`/`classification` are computed server-side after create (`computeAndClassify()`) — an optional `classification: 'sql'` in the body is honored as a manual override, any other value is ignored in favor of the auto mql/none result. **`referred_by_type`/`referred_by_id` (added 2026-09-15)**: optional, both-or-neither (`422` if only one is set) — captures which existing Company or Contact referred this Lead in, typically alongside `source: "Referral"` but not enforced to only that source. `referred_by_type` must be `"company"` or `"contact"` (reuses `ActivityRelatedType`'s values, restricted to just those two — Deal/Prospect aren't valid referrers). **Updated 2026-09-16**: `referred_by_id` is now existence-checked too (`404` if the Company/Contact it points at doesn't exist), same `validateReferredBy` function extended with a `db` param. Same both-or-neither/type/existence validation applies to `PUT /leads/:id` below. Now surfaced in a report — `GET /reports/top-referrers` (§8.4, FR-CRM-121) groups Leads by referrer and joins through to their resulting Deals' Won status/revenue. |
| `GET` | `/leads/:id` | 🟢 | Single lead. |
| `PUT` | `/leads/:id` | 🟢 | Update (including status transitions). **Not a true partial update** — every `leadForm` field (`name`, `company_id`, `email`, `phone`, `source`, `status`, `notes`, `assigned_to`) is overwritten from whatever the request body contains, so a caller must resend the full current record, not just the changed field(s) — note this means omitting `company_id` clears the link entirely, unlike Contact's Update, which only touches `company_id` if non-zero/present. `score` is always recomputed. `classification` is the one exception with real partial-update semantics: omitting it (or sending anything other than `'sql'`) leaves an existing `'sql'` override in place and otherwise falls through to the auto mql/none result — only an explicit `classification: 'sql'` in the body sets/keeps it. Frontend: `pages/crm/leads/[id].vue`'s "Mark Sales Ready" button. **2026-09-10** (FR-CRM-113): when the resolved `status` differs from the pre-request value, also logs a company-scoped Activity in the same transaction (`utils.LogStatusChangeActivity`, shared with Prospect's own `Update`), using whichever of the post- or pre-mutation `company_id` is set — covers the case where a Company was just linked on this same save. **Superseded 2026-09-22 for the Kanban drag-move specifically** — see `PATCH /leads/:id/status` below; this full-record `PUT` used to be (riskily) reused for that too. |
| `PATCH` | `/leads/:id/status` | 🟢 | **Added 2026-09-22** (FR-CRM-122). Body: `{ status: LeadStatus, position?: number }`. Dedicated narrow-PATCH endpoint for the Kanban drag-and-drop quick-move, mirroring `PATCH /deals/:id/stage` (§7.1) — unlike the full-record `PUT` above, this only ever touches `status`/`position`, so a drag-move never risks blanking the rest of the record. `position` semantics are identical to `PATCH /deals/:id/stage`'s: applied as-is when present (the frontend's own computed drop-index within the destination lane); when omitted and `status` actually changed, appends to the end of the destination lane instead (`MAX(position)+1`). Also logs the same company-scoped Activity as `PUT /leads/:id` on an actual status change (`utils.LogStatusChangeActivity`). |
| `DELETE` | `/leads/:id` | 🟢 | Soft-delete (sets `deleted_at`/`deleted_by`) — recoverable, see below. |
| `GET` | `/leads/trash` | 🟢 | Admin/Sales Manager only. Paginated list of soft-deleted Leads (same envelope as `GET /leads`). Backs `pages/admin/trash.vue`. **Added 2026-09-10**: `?search=` (case-insensitive, matches `name`) — see the `GET /companies/trash` row below for the shared implementation. |
| `POST` | `/leads/:id/restore` | 🟢 | Admin/Sales Manager only. Clears `deleted_at`/`deleted_by`. |
| `GET` | `/leads/:id/score-breakdown` | 🟢 | **Added 2026-09-11**, `FR-CRM-007`. Same gating as `GET /leads/:id` (no extra role restriction beyond `authed`). Recomputes `computeLeadScoreDetailed()` (a small refactor of `computeLeadScore` that also returns which `LeadScoringCriterion` rows matched, not just the sum) live from the Lead's current fields — always consistent with `Lead.score` even if criteria changed since the Lead was last saved. Response: `{ score: number, threshold: number, classification: LeadClassification, matched: {id, name, field, weight}[] }`. Exists because `GET /admin/lead-scoring-criteria` (§8.7) is Admin-only, so a Sales Rep viewing their own Lead had no way to see *why* it scored what it did — only the final number/badge. Frontend: a "How is this calculated?" info-icon popover next to the score badge on `pages/crm/leads/[id].vue`, fetched lazily on first open. |
...
interface Prospect {
  ...
  business_unit: 'Project' | 'Product' | null
  business_unit_item: string | null
  // Added 2026-09-22 (FR-CRM-122) — orders this Prospect's Kanban card within
  // its own status lane only, never compared across lanes. Server-computed:
  // see Deal.position's doc comment (§7.1) for the full scheme; set here via
  // PATCH /prospects/:id/status below.
  position: number
  // Added 2026-09-23 (FR-CRM-123) — when this Prospect entered its current
  // status lane, same rules as Lead.stage_entered_at.
  stage_entered_at: string | null
  // Added 2026-09-24 — the lane this record left on its last stage/status
  // change (set with stage_entered_at); null if it has never moved.
  previous_stage: string | null
...
| Method | Path | Status | Description |
|---|---|---|---|
| `GET` | `/prospects` | 🟢 | Filters: `status`, `source`, `assigned_to` (`unassigned` matches `IS NULL`), `company_id` (exact match), `search` (name/email/company name, via the same `LEFT JOIN companies` pattern as `GET /leads`), `exclude_converted=true` (`converted_lead_id IS NULL`). `sort=company_name`/`-company_name` also joins to `companies`; otherwise accepts `created_at` (default, descending) / `name` / **`position`** (added 2026-09-22, FR-CRM-122 — used by the Kanban board so a within-lane reorder's order survives a refetch). Backs `pages/crm/prospects/index.vue`. |
...
| `PUT` | `/prospects/:id` | 🟢 | Update (including status transitions). Not a true partial update — every field is overwritten from the request body, same as `PUT /leads/:id`. Same `status: 'Converted'` guard as Create: rejected with `422` unless the Prospect is already `Converted` (a client harmlessly resubmitting an unchanged record's status is allowed through — only an attempted *transition* into `Converted` from anything else is blocked). **2026-09-10** (FR-CRM-113): when the resolved `status` differs from the pre-request value, also logs a company-scoped Activity in the same transaction (`utils.LogStatusChangeActivity`, shared with Lead's own `Update`) — a status move into `Disqualified` counts as contact too, same as any other transition. **Superseded 2026-09-22 for the Kanban drag-move specifically** — see `PATCH /prospects/:id/status` below; this full-record `PUT` used to be (riskily) reused for that too. |
| `PATCH` | `/prospects/:id/status` | 🟢 | **Added 2026-09-22** (FR-CRM-122). Body: `{ status: string, position?: number }`. Dedicated narrow-PATCH endpoint for the Kanban drag-and-drop quick-move, mirroring `PATCH /leads/:id/status` above — unlike the full-record `PUT` above, this only ever touches `status`/`position`, so a drag-move never risks blanking the rest of the record. Same `status: 'Converted'`/active-stage guards as `PUT`/Create, and the same `position` append-on-omit fallback as `PATCH /deals/:id/stage` (§7.1). Also logs the same company-scoped Activity as `PUT /prospects/:id` on an actual status change. |
...
interface Company {
  ...
  // WHERE related_type='company', not a stored column — null if the Company
  // has no company-scoped Activity at all. Since 2026-09-10 (FR-CRM-113/114)
  // this also advances from a Prospect/Lead/Deal stage change and from a
  // manually backdated Activity, not only a same-day manual log — see §7.2.
...
| `GET` | `/contacts/trash` | 🟢 | Admin/Sales Manager only. Paginated list of soft-deleted Contacts (same envelope as `GET /contacts`). Backs `pages/admin/trash.vue`. **Added 2026-09-10**: `?search=` (case-insensitive, matches `name`) — see the `GET /companies/trash` row above for the shared implementation. |
...
interface Deal {
  ...
  // Added 2026-09-22 (FR-CRM-122) — orders this Deal's Kanban card within its
  // own stage lane only, never compared across stages. Server-computed:
  // MAX(position)+1 in its lane on create, recomputed as the midpoint of its
  // new neighbors' position on every drag-move (see PATCH /deals/:id/stage
  // below). Never set directly by a create/update form.
  position: number
  // Added 2026-09-23 (FR-CRM-123) — when this Deal entered its current
  // stage, same rules as Lead.stage_entered_at (PUT/PATCH stage).
  stage_entered_at: string | null
  // Added 2026-09-24 — the lane this record left on its last stage/status
  // change (set with stage_entered_at); null if it has never moved.
  previous_stage: string | null
...
> **Unified pipeline board:** `pages/crm/deals/index.vue`'s Kanban also renders unconverted Leads (`GET /leads?exclude_converted=true`, §3) as cards in the `Lead`/`Qualified` columns, plus Disqualified Leads in `Lost`. Dragging a Lead card within `Lead`/`Qualified`/`Lost` just updates its `status` (`PATCH /leads/:id/status`, §3 — **changed 2026-09-22**, previously the full-record `PUT /leads/:id`); dragging it into `Proposal Sent`, `Negotiation`, or `Won` instead fires `POST /leads/:id/convert` (§3) with `stage` set to the dropped column, turning the card into a real Deal in place. Every drag on this board — Deal or Lead, same lane or a different one — also carries a `position` (§7.1's `PATCH /deals/:id/stage` row, §3's `PATCH /leads/:id/status` row) so within-lane order persists too, not just which lane a card is in.
...
| `PUT` | `/deals/:id` | 🟢 | Full update. **2026-09-10** (FR-CRM-113): when the submitted `stage` differs from the Deal's current one, also logs a company-scoped Activity (`type: 'note'`, subject "Deal stage changed: X → Y") inside the same transaction as the existing `stage_changed` audit-log write — same `oldStage != deal.Stage` gate, so an unrelated field edit doesn't fire it again. |
| `PATCH` | `/deals/:id/stage` | 🟢 | Body: `{ stage: DealStage, position?: number }`. Dedicated endpoint for the Kanban drag-and-drop (`CrmPipelineBoard`'s `@move`) so the backend can also update `status` (open/won/lost) in one transaction. **Added 2026-09-22** (FR-CRM-122): `position` is the frontend's own computed drop-index within the destination stage lane (fractional — the midpoint of its new neighbors' `position`), applied as-is when present; when omitted (the mobile dropdown-move, which has no drag geometry to compute one from) and the stage actually changed, the backend appends to the end of the destination lane instead (`MAX(position)+1`). **Does not** fire an auto Customer-Product creation on `Won` — `FR-CRM-064` was confirmed 2026-08-22 as intentionally not built (see §8.2 below); CustomerProduct creation stays a manual, client-triggered action (FR-CRM-065) regardless of stage transitions. **FR-CRM-045** (added 2026-08-22): when `AppSettings.require_signed_contract_before_won` (§8.7a) is enabled, moving *into* Won 422s (`"a signed contract is required before marking this deal Won"`) unless the Deal already has ≥1 Contract with `status: 'signed'` — checked before `status`/`stage` are saved, so a blocked transition mutates nothing. The check only fires on the actual transition (the Deal's stored `status` wasn't already Won) — re-saving a Deal that's already Won (e.g. editing an unrelated field) does not re-check, so enabling the toggle later never retroactively locks editing of pre-existing Won deals that predate it. `POST /deals` enforces the same check unconditionally when the submitted `stage`/`status` resolves to Won (a brand-new Deal has no prior state to compare against). `PUT /deals/:id` enforces it with the same transition-only guard as `PATCH /deals/:id/stage`. **2026-09-10** (FR-CRM-113): same company-scoped Activity side-effect as `PUT /deals/:id` above, on the same `oldStage != deal.Stage` gate — moving into Lost counts as contact too, no different treatment than any other stage. **Updated 2026-09-24:** also accepts an optional `lost_reason` (`price`/`timing`/`competitor`/`no_budget`/`other`, else 422). When sent with a move into a Lost stage it's saved on the Deal (the Overview Pipeline's side panel asks for it); ignored for any other stage; still optional, so the Kanban drag is unchanged. The Deals Kanban now also sends `lost_reason` on a drag into Lost, after asking for it. |
...
| `GET` | `/deals/trash` | 🟢 | Admin/Sales Manager only. Paginated list of soft-deleted Deals (same envelope as `GET /deals`). Backs `pages/admin/trash.vue`. **Added 2026-09-10**: `?search=` (case-insensitive, matches `title`) — see the `GET /companies/trash` row above for the shared implementation. |
...
```ts
type ActivityType = 'call' | 'email' | 'meeting' | 'note'   // 'note' added 2026-09-10 (FR-CRM-113/114) — covers both a manually-logged freeform note and a system-generated entry (a Prospect/Lead/Deal stage/status change, or a Company backdated "mark as contacted")
...
| `PUT` | `/quotes/:id` | 🟢 | **Widened 2026-08-23** — updates every field above except `id`/`deal_id`/`number`/`file_*`/`uploaded_at`: `items` (incl. per-item `discount_percent`), `scope_of_work`, `validity_date`, `status`, `reference_number`, `issue_date`, `credit_days`, `price_type`, `vat_enabled`, `wht_enabled`, `wht_rate`, `discount_total`, `notes`, `internal_notes` (`stores/quotes.ts`'s `QuoteUpdatePayload`). Existed on the backend before this rebuild but had no UI caller until the new editor page. **No transition validation** — any `status` value in `models.ValidQuoteStatuses` (`draft`/`sent`/`accepted`/`rejected`) is accepted regardless of the Quote's current stored status (e.g. `accepted` → `draft` is not blocked); the frontend's own gating (below) is the only guard. |
...
> **"Send to Customer" (added 2026-09-10) is frontend-only, same `PUT /quotes/:id` underneath.** `pages/crm/quotes/[id].vue` now has a dedicated, confirmed "Send to Customer" action (shown only while `status === 'draft'`) instead of `status → sent` being just another field on the generic Save button — it still calls the same `PUT` with `status: 'sent'` plus whatever else the form currently holds, and shows a confirmation dialog first since sending is presumed one-way (the app never emails the Quote itself; the confirmation copy says so). This is a UI-only gate, not a new backend rule — see the "No transition validation" note on the `PUT /quotes/:id` row above.

> **Uploaded (PDF) Quotes can now also change status**, via a small inline status `<select>` on `pages/crm/deals/[id]/quotes.vue` (the Deal's Quotes tab) — previously only a manually-built Quote's own editor page (`pages/crm/quotes/[id].vue`, items-only) exposed the `status` field at all, so an uploaded Quote could never leave `draft` from the UI. `quotesStore.updateStatus(id, status)` (`stores/quotes.ts`) rebuilds the full `QuoteUpdatePayload` from the already-loaded Quote (items/scope_of_work/etc. unchanged) since `PUT /quotes/:id` has no lighter partial-update mode, then calls `update()` as normal.
...
| `POST` | `/deals/:dealId/payment-installments/bulk` | 🟢 **added 2026-09-16** | Body: `{ installments: [{amount, due_date, note}] }` — creates every row in one batch insert + one summary audit-log entry (`bulk_created_payment_installments`), mirroring `CampaignHandler.BulkCreateTasks`'s shape, instead of the frontend looping N calls to the single-row `POST` above (which would also write N separate audit-log rows for one user action). The frontend computes the actual split (equal amounts, spaced due dates) — this endpoint only validates each row with the same per-row rules as Create and inserts; non-empty array required, the whole batch fails together if any row is invalid (no partial insert). Backs the "Generate Schedule" modal on the Payment Schedule section. |
...
| Method | Path | Status | Description |
|---|---|---|---|
| `GET` | `/tasks` | 🟢 | Filters: `related_type`+`related_id`, `status`, `assigned_to`, `campaign_id` (added 2026-09-04, §7.7). `status=pending` powers the dashboard's "Upcoming Follow-ups" widget across all related records — support that query without requiring `related_type`/`related_id`. **2026-09-04**: filtering logic extracted into `applyTaskFilters` (`internal/handlers/filters.go`), matching the existing `applyCompanyFilters`/`applyDealFilters` pattern — behavior unchanged for the filters that already existed. **2026-09-25 (server-side paging for `/crm/tasks`):** also accepts `related_type` alone (every task on that kind of record); `assigned_to=unassigned`; `search` (ILIKE on title/description **or** the linked record's display name — Deal title, Contact/Company/Prospect/Lead name; `%`, `_` and `\` match literally, as in every search filter since 2026-09-25); `business_unit` (tasks whose linked Deal/Prospect/Lead has that business unit — Contact/Company-linked tasks never match); `due_from` (inclusive) / `due_before` (exclusive) bounds on `due_date`, each an RFC 3339 timestamp (the Tasks page sends the viewer's local midnight as a UTC instant, so "today" is the viewer's today) or a bare `YYYY-MM-DD` (UTC midnight) — an unparseable value is a 422. `sort` allows `created_at` (default `-created_at`), `due_date`, `title`. The Tasks page issues one query per due-date group (Overdue: `status=pending&due_before=<today>`; Today: `due_from=<today>&due_before=<tomorrow>`; Upcoming: `due_from=<tomorrow>`; Done: `status=done`), each paged independently. |
...
> **Contract status is now editable after creation (added 2026-09-10)**, not just set once via `CrmAddContractModal` at create time — `contracts.vue` now has an inline status `<select>` per Contract row calling `contractsStore.update(id, { status })` (`PUT /contracts/:id`, already existed on the backend but had no post-creation UI caller until now), and still triggers the same `FR-CRM-048` create-Project prompt above when the new status is `signed`. **Updated 2026-09-16**: `status` is now validated server-side on both `Create` and `Update` — `models.IsValidContractStatus` (mirroring `Payment`'s own `IsValidPaymentMethod`), `422` on anything outside `draft`/`sent`/`signed`/`expired`. `quote_id`, if set, is likewise now checked to exist and to belong to the same Deal as the Contract (previously accepted any id, even one from a different Deal entirely, or one that didn't exist at all) — `404`/`422` respectively via the new shared `validateContractForm` (`internal/handlers/contracts.go`), called from both `Create` and `Update`.
...
All nine report endpoints are Admin/Sales-Manager only (`RequireRoles`) and have real frontend pages under `pages/crm/reports/` (a small landing page linking to each, gated the same way):
...
| `GET` | `/reports/top-referrers?assigned_to=&date_from=&date_to=` | **Added 2026-09-16.** `FR-CRM-121`. Per referring Company/Contact (`Lead.referred_by_type`/`referred_by_id`, `FR-CRM-119`), how many Leads they referred, how many became Deals (`deals.lead_id`), how many of those Deals were Won, and total Won revenue — `{ referrer_type, referrer_id, referrer_name, leads_referred, deals_created, deals_won, won_revenue }[]`. Resolves the referrer's display name via a double-`LEFT JOIN` against `companies`/`contacts` keyed by `referred_by_type`, since the referrer is polymorphic and no single FK column points at "whichever table type says." `assigned_to`/date-range filter on the *referred Lead's* own fields, same convention as `lead-source-conversion` above. `GET .../export` streams the same rows as CSV. |
| `GET` | `/reports/prospect-source-conversion?assigned_to=&date_from=&date_to=` | **Added 2026-09-03.** `FR-CRM-107`. Conversion rate by `Prospect.source` — exact structural mirror of the row above, one funnel stage earlier: "converted" = `status = 'Converted'` (the same terminal state `Prospect.converted_lead_id` being non-null implies). **Admin/Marketing/Sales Manager** — its own `RequireRoles` group, separate from the Admin/Sales-Manager-only `reports` group every other row on this page lives under, since Marketing has no access to any Deal/Lead-derived report but does own this one. `GET .../export` streams the same rows as CSV, matching every other report's export pattern. **Refactored 2026-09-03 (later)** — shares a `fetchSourceConversion` query helper + `conversionRate` calc with `lead-source-conversion` above (backend `reports.go`), since the two queries were identical apart from model/success-status. No behavior change. |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| `GET` | `/audit-log` | Admin/Sales Rep/Sales Manager | Filters: `entity_type`, `entity_id`, `actor_id`, date range — Admin only; a non-Admin caller's `entity_type`/`actor_id` are ignored (see below), `entity_id`/date range still apply. Must be **append-only** at the storage layer (`NFR-007`) — no `PUT`/`DELETE` route should exist for this resource at all. **2026-09-25:** `action` (e.g. `stage_changed`) is now honored for every role — it narrows within the role's visible slice and can never widen it. It was previously ignored, so the Activities pages' `action=stage_changed` call also got back every other Deal audit row for an Admin (and `reassigned` rows for a Sales Manager), none carrying `after.stage`, which rendered as a bare "Stage set:" with nothing after the colon. |

At minimum, write an entry whenever: a Deal's `stage` changes, a Deal's `status` becomes `won`/`lost`, or a `CustomerProduct`/`Project` `status` changes (per `FR-CRM-082`'s explicit minimum scope) — all four are now implemented (`deals.go` for the Deal events, `projects.go`/`products.go` for the other two). The frontend's `/admin/activity-log` page is already repointed at this real endpoint (Admin only — the non-Admin restriction below has no dedicated UI of its own beyond the two Deal-detail consumers noted next).

**Route-gated to Admin/Sales Rep/Sales Manager, but `List()` hard-restricts what a non-Admin caller actually gets back** (`internal/handlers/auditlog.go`), regardless of what `entity_type`/`actor_id` they pass: always `entity_type=deal`, and a further split by role — Sales Rep gets `action=stage_changed` only, Sales Manager also gets `reassigned`/`bulk_reassigned`. This backs two Deal-detail consumers: (1) Deal stage-change history surfaced as read-only context, and (2) `entity_type`+`entity_id` filtering (already supported above) backing the Deal detail page's "Owner History" card (`pages/crm/deals/[id]/index.vue`, Admin/Sales Manager only), which calls `GET /audit-log?entity_type=deal&entity_id=:id&per_page=200` and filters client-side to `reassigned`/`bulk_reassigned` actions to show that one Deal's prior owners. **Updated 2026-09-09** — Owner History was Admin-only until a Sales Manager (the one who actually performs reassignments, per `FR-CRM-025`/M-8) was given the same `reassigned`/`bulk_reassigned` access as Admin. The explicit `per_page=200` override (the store's own default is 20, sized for the paginated Activity Log list view) exists because there's no server-side `action` filter to narrow the query to just reassignments — without it, a Deal with more than 20 total audit-log rows of any action type could silently drop older reassignments off this card. Same class of "unbounded-but-capped" limitation as NFR-003 below, just smaller-radius.

**Found and resolved 2026-09-09, swept across seven config-list endpoints the same day:** `GET /admin/pipeline-stages`, `GET /admin/lead-sources`, `GET /admin/product-categories`, `GET /admin/prospect-sources`, `GET /admin/industries`, `GET /admin/company-sizes`, `GET /admin/revenue-sizes`, and `GET /admin/job-titles` were each bundled into the same Admin-only route group as their own Create/Update/Delete (`adminOnly` middleware), even though every one of them backs a plain dropdown/filter needed by pages no role restriction gates: Deal/Lead/Contact/Company/Project create-and-edit forms, the shared Dashboard, the Deals Kanban board, and — the worst instance — Marketing's own primary `pages/crm/prospects/*` pages (source dropdown/filter). Any non-Admin role loading one of these got a silent 403, which the frontend's axios interceptor (`plugins/axios.ts`) turns into a hard `router.push('/')` — yanking the user back to the Dashboard away from the very page they were trying to use, Marketing's own Prospects page included. List is now registered directly on `authed` (open to every authenticated role) for all seven; Create/Update/Delete stay behind the `adminOnly` group, unchanged. Regression-guarded by `TestRBAC_PipelineStagesLeadSourcesListOpenWritesAdminOnly` and `TestProspectSources_ListOpenWritesAdminOnly` (`tests/rbac_test.go`, `tests/prospect_source_test.go`). Also fixed the same day: `PUT /leads/:id` and `POST /leads/:id/convert` had no role check at all (any authenticated role, including Marketing/Production, could mutate or convert a Lead they have no business touching) — now `Admin`/`Sales Rep`/`Sales Manager` only, matching the frontend's own Mark SQL/Convert to Deal buttons, which were previously hidden only by convention (`TestRBAC_LeadUpdateConvertSalesPipelineOnly`).
...
`pages/admin/pipeline-config.vue` moved from one long scrolling page of stacked `UCard` sections to a tabbed layout on 2026-08-21 (pure frontend template reorganization, no API/store/handler changes), since grown to six tabs as new config sections were added: **Pipeline Stages**, **Sales Quota & Targets** (quota + per-quarter targets), **Lead Sources & Scoring** (lead/deal sources + lead scoring criteria), **Prospect Stages & Sources**, **Company & Product Data** (industries/sizes/job titles/product categories), **Notification Rules**. Local-`ref` tabs (`activeTab`), same as `pages/admin/trash.vue` — not the route-backed child-route pattern `pages/crm/deals/[id].vue` uses (this page has no natural sub-routes), and not `pages/crm/companies/[id].vue`'s `?tab=`-synced tabs either (see the 2026-09-10 note just below — this page's own `?tab=` only seeds once, on mount, and never rewrites the URL as the rep switches tabs afterward). The "Related Configuration" (Tags) banner stays above the tabs, since it's a nav-out link, not a config section of its own.

**`pages/crm/companies/[id].vue`'s tabs are two-way `?tab=`-synced (added 2026-09-10)**, via `useQuerySyncedRef('tab', 'overview')` (`composables/utils/useQuerySyncedRef.ts`) rather than this page's one-time-seed convention above or a plain local `ref` — a refresh, back-button return, or shared link now lands back on whichever tab (Contacts/Deals/Quotes & Contracts/Products/Projects/Activity/Tasks/Attachments) the rep had open, not always Overview. `pages/crm/deals/[id].vue` still doesn't need this: its tabs are already route-backed child pages (`pages/crm/deals/[id]/quotes.vue` etc.), so the URL already reflects the active tab with no extra plumbing. **`pages/crm/projects/index.vue`'s Projects/Products tab switch was converted the same way (also 2026-09-10)** — it was a plain local `ref('projects')`, the one remaining `UTabs` usage in `pages/crm/` with no URL persistence at all (not even the one-time-seed convention); now `useQuerySyncedRef('tab', 'projects')`, same two-way behavior as Company detail's tabs.
...
>
> **`pages/crm/tasks/index.vue` gained client-side pagination (added 2026-09-10).** `tasksStore.fetchAll()` itself is unaffected by this note (Tasks aren't part of the 200-row-cap sweep above — team-wide Task volume is expected to stay well under it), but the list page was rendering every fetched-and-filtered Task in one unbroken `CrmTaskList`, with no page controls at all — the one remaining list-page-shaped view with that gap; every other list page already paginated one way or another. Fixed with `useTablePagination` (`composables/utils/useTablePagination.ts`), the same client-side-slice composable `pages/crm/projects/index.vue` already uses for its Projects/Products tabs, plus a `TablePagination` component instance below the list — not a `fetchList()`/`useServerListPage` conversion, since Tasks stays on `fetchAll()` per above. |
