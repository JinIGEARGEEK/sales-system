...
> **Contract status is now editable after creation (added 2026-09-10)**, not just set once via `CrmAddContractModal` at create time — `contracts.vue` now has an inline status `<select>` per Contract row calling `contractsStore.update(id, { status })` (`PUT /contracts/:id`, already existed on the backend but had no post-creation UI caller until now), and still triggers the same `FR-CRM-048` create-Project prompt above when the new status is `signed`. **`PUT /contracts/:id` has no server-side validation at all on `status`** — not even enum-membership checking (`internal/handlers/contracts.go`'s `Update` assigns `form.Status` directly whenever non-empty, unlike Quote's `IsValidQuoteStatus` check) — so this inline `<select>`'s own fixed option list (`CONTRACT_STATUS_OPTIONS`: `draft`/`sent`/`signed`/`expired`) is the only thing keeping a Contract's stored status a recognized value; a direct API call could still write anything.
