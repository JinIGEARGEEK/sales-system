...
| `CrmStatCard` | Dashboard stat tile (`UCard` + label/value/hint) | `valueClass`/`hintClass` props for conditional coloring (e.g. green when healthy, red when not). |
| `CrmActivityTimeline`, `CrmPipelineBoard`, `CrmTeamMemberSelect`, `CrmConfirmDeleteModal` | Domain-specific composed components | `PipelineBoard`'s `STAGE_COLORS` map (hand-authored, not derived from `constants/mockData.ts`) is a known fragility — see §10. |
...
- **`PipelineBoard.vue`'s `STAGE_COLORS` map is hand-synced with `constants/mockData.ts`'s `DEAL_STAGE_OPTIONS`** — there's no shared source of truth. Adding/renaming a deal stage in mock data without updating `STAGE_COLORS` silently falls back to `FALLBACK_COLOR` (`var(--color-primary)`, i.e. dark navy) instead of erroring. If you add a stage, update both.
