...
- **Collapsible nav groups (added 2026-09-11)**: `layouts/default.vue`'s `MenuItem[]` array can also hold a `MenuGroup` entry (`{key, icon, label, children: MenuItem[]}`, discriminated via `isMenuGroup()`/`'children' in entry`) instead of a plain link — used for the first time to fold Pipeline Config/Staff/API Keys under one "Admin" group, as the flat Admin-only tail of the nav kept growing (§ the 2026-09-01 note above already called out the list getting long). Renders as a `<button>` header (same `sidebar-nav-link` classes as a plain item, plus a `chevron-right` icon that rotates 90° when expanded) followed by its children indented one level (`pl-4`), only when expanded. Expand/collapse state is a per-viewer UI preference in `localStorage` (`sidebar-collapsed-groups`, a `Set<string>` of collapsed group keys, JSON-serialized) — not a store/backend field, same "client-only, `import.meta.client`-guarded" convention `composables/utils/useAuth.ts` already uses for its own localStorage reads. A group whose current route matches one of its children always renders expanded regardless of the persisted flag (`isGroupExpanded()` ORs the two), so landing directly on e.g. `/admin/api-keys` never hides which nav section you're in. Both the desktop `<aside>` and the mobile `<USlideover>` nav lists render groups identically (same duplication precedent as everything else in this file — the two nav lists have always been hand-kept in sync, not componentized).
