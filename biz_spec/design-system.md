...
**Scope:** Visual design tokens, layout conventions, component patterns, and UI rules actually implemented in this codebase, as of 2026-08-13.
...
| `CrmStatCard` | Dashboard stat tile (`UCard` + label/value/hint) | `valueClass`/`hintClass` props for conditional coloring (e.g. green when healthy, red when not). Optional `icon`/`iconClass`/`iconBgClass` render a colored icon chip; optional `accentGlassClass` renders a frosted-glass gradient strip down the card's left ~10% (the root `UCard` has `overflow-hidden` specifically so this strip is clipped to the rounded corners). Currency values use `useFormatter`'s `priceFormatCompact` (`0,0.[0]a` → `5.2M`, `77.8K`) prefixed with the `crm.dashboard.currencySymbol` locale key (`฿`), not a trailing "THB"/"บาท" unit span — a stat tile is meant to be read at a glance, not to full precision; `priceFormat` (full 2-decimal precision) stays reserved for detail/export contexts. |
| `CrmActivityTimeline`, `CrmPipelineBoard`, `CrmTeamMemberSelect`, `CrmConfirmDeleteModal` | Domain-specific composed components | `PipelineBoard` resolves a stage's color by first checking the hand-authored `DEAL_STAGE_COLORS` fallback map (`constants/mockData/deals.ts`, covering only the original default stages), then the Admin-configurable `stores/pipelineStages.ts` (`pages/admin/pipeline-config.vue`), then `FALLBACK_COLOR` — see §10 for the remaining fragility. |
...
| `CrmImportContactsModal` | FlowAccount "สมุดรายชื่อ" (address book) bulk-import modal for Companies/Contacts | Parses CSV/XLS/XLSX via the `xlsx` (SheetJS) package. Matches columns by exact Thai header text (not position) against a hardcoded `HEADER_MAP`, so it only works for that specific FlowAccount export shape — see §8 and §10. |

---

## 8. State Management: Entity Stores

CRM entities that need to be created/edited/deleted from more than one page go through a Pinia store under `stores/`, not a local `ref([...MOCK_X])` copied at page setup. This exists specifically because of a real bug class: before `stores/companies.ts` and `stores/contacts.ts` existed, every page (`crm/companies/*`, `crm/contacts/*`, `crm/deals/*`, `pages/index.vue`) independently seeded its own local copy of `MOCK_COMPANIES`/`MOCK_CONTACTS` — so a company created on one page was invisible everywhere else (e.g. a new company wouldn't appear in the Deal-creation company picker), and `companyNameById()` (a plain function reading the static array) couldn't resolve names for anything created after page load.

**The pattern** (see `stores/companies.ts` / `stores/contacts.ts`):
- `state.items` seeded once from the matching `MOCK_*` array in `constants/mockData.ts`.
...
- `add(entity): number` returns the new numeric id (computed as `max(existing ids) + 1`); `remove(id)`.
...
- Edit/detail pages get the live record via `computed(() => store.items.find(...))` and mutate its properties directly in `onSave` (Pinia state is reactive, so this is safe) rather than reassigning a disconnected local copy.

**Known gap:** Leads, Deals, and Tags have **not** been migrated to this pattern yet — they still use page-local `ref([...MOCK_X])` copies exactly like Companies/Contacts did before this fix. This means the same cross-page-invisibility bug still exists for those three entities today (e.g. a Deal created on one tab session is only visible on that same page instance). If a future feature needs Leads/Deals/Tags to be visible/editable consistently across pages (bulk import, cross-entity linking, etc.), migrate them to the same store pattern first — don't build on top of the local-ref approach.
...
- **Leads, Deals, and Tags are not yet on the entity-store pattern** (§8) — only Companies and Contacts are. They still use page-local `ref([...MOCK_X])` copies, so records created on one page instance aren't visible from other pages. Not a regression to "fix" reflexively — just be aware before building a feature (import, cross-linking) that assumes otherwise.
