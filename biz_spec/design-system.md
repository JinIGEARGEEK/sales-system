...
### 2.5.2 Highlighted field group inside a form grid — `border-sky-300 bg-sky-50`

Added 2026-09-03 for the Business Unit select + its Project/Product item picker (`CrmBusinessUnitItemField`) on the Deal/Lead/Prospect create/edit forms — the first instance in this codebase of visually setting a couple of related fields apart from the plain fields around them inside a `grid grid-cols-*` form. Wrap the group in its own `md:col-span-2` cell containing a nested `grid grid-cols-1 gap-3 md:grid-cols-2`, styled `rounded-lg border border-sky-300 bg-sky-50 p-3`. Don't reach for a semantic status token here (`--color-warning-bg`/`--color-info-bg`/`--color-success-bg`/`--color-error-bg`) — those are reserved for actual alert/status meaning (`UAlert`, badges, toasts) and would misleadingly suggest an error/warning/info state on an ordinary field pair. This doesn't apply to a toggle row (see `biz_spec/ux-ui-guidelines/modal.md`'s explicit "no border/bg around a toggle row" rule) — that's a different UI element with its own established convention.

**Updated 2026-09-03**: this box originally used `border-[var(--color-primary)]/30 bg-[var(--color-primary-bg)]`, matching `CrmStatusPill`'s active-state pairing. That read as a flat gray box, not a highlight — `--color-primary` is a dark navy (§2.1) mixed at only ~10% opacity for `--color-primary-bg`, so at this box's scale (a whole field group, not a thin pill border) there wasn't enough saturation left to register as color at all. Same root cause §2.6 already called out for progress-bar fills — switched to a plain Tailwind `sky-300`/`sky-50` pairing instead of a `--color-*` token for the same reason §2.6 gives: don't tie a non-primary visual accent to `--color-primary` and expect it to still look blue. All 6 call sites (`pages/crm/{deals,leads,prospects}/create.vue` and their `[id]` detail-edit counterparts) were updated together — if a 7th form gains this same grouping treatment, match `sky-300`/`sky-50`, not the old primary-token version.
...
`UTabs`' `list` slot is likewise overridden globally, in the same `app.config.ts`, to the app's glass-pill look (`bg-white/65 backdrop-blur-2xl border border-white/90 divide-x divide-white/70`), applying to all 7 `UTabs` call sites (dashboard tab switcher, Deal/Company/Guideline/pipeline-config/trash detail tabs, Projects' Products/Projects switcher) at once. **Fixed 2026-09-03**: that override also carried `shadow-xl`, which on a thin tab strip (no card body beneath it to visually anchor a shadow that large) spread out into a conspicuous hazy gray box sitting under the tabs rather than reading as a shadow — dropped to `shadow-sm`. Don't bump it back up without checking how it renders on an actual tab bar, not just in isolation.
...
- **Sidebar** (`<aside>`, desktop only, `md:w-44`): dark glass panel — `bg-[var(--color-sidebar-bg)]/90` + `backdrop-blur-2xl`, with a subtle top-left sheen overlay (`bg-linear-to-br from-white/10 via-transparent to-transparent`). Contains, top to bottom: brand row (hub icon in `text-yellow-400` + "CRM System", `h-(--layout-header-height)`), nav list, then a footer block. **Updated 2026-09-01**: the footer was minimized to help the growing menu list (15 items after Prospects, §3.1a) fit without scrolling — it now shows avatar + name only (the email line was dropped; still available via a native `title` tooltip on hover over the row) at tighter padding (`p-2`, `mt-1.5`/`pt-1.5` between the name row and the `SwitchLang`+logout row below it), down from the original `p-2.5`/`mt-2.5`/`pt-2.5` + two-line avatar block.
- **Nav items**: `sidebar-nav-link flex items-center gap-3 rounded-lg px-3 py-1.5 text-xs text-white` (tightened from `py-2 text-sm` on 2026-09-01, same reason as the footer trim above) — active state adds the `is-active` class (`sidebar-nav-link.is-active` in the component's `<style scoped>` block, a yellow-to-blue gradient glow ring + `font-medium`, not a plain `bg-white/10`). Icon gets `size-5 shrink-0`, label gets `truncate` — this is deliberate so long labels never wrap to two lines (a real bug that was fixed; keep both classes on any new nav item). Separators between role-based groups (e.g. before Prospects, after Prospects, after Tags) use `<USeparator class="my-0.5 opacity-10" />`.
- **Collapsible nav groups (added 2026-09-11)**: `layouts/default.vue`'s `MenuItem[]` array can also hold a `MenuGroup` entry (`Omit<MenuItem, 'path'> & {key, children: MenuItem[]}` — derived via `Omit` rather than hand-repeating icon/label/separator/roles, so a future `MenuItem` field reaches `MenuGroup` too without a matching manual edit; discriminated via `isMenuGroup()`/`'children' in entry`) instead of a plain link — used for the first time to fold Pipeline Config/Staff/API Keys under one "Settings"/"ตั้งค่า" group, as the flat Admin-only tail of the nav kept growing (§ the 2026-09-01 note above already called out the list getting long). Renders as a `<button>` header (same `sidebar-nav-link` classes as a plain item, plus a `chevron-right` icon that rotates 90° when expanded, `aria-expanded`/`aria-controls` wired to the children container's `id`, and the same active-state class a plain nav item gets when the current route matches one of its children) followed by its children indented one level (`pl-4`), only when expanded. Expand/collapse state is a per-viewer UI preference persisted via `composables/utils/useCollapsedNavGroups.ts` (`localStorage` key `sidebar-collapsed-groups`, a `Set<string>` of collapsed group keys, JSON-serialized, both the read and the write try/catch-guarded — not a store/backend field, same "client-only, `import.meta.client`-guarded" convention `composables/utils/useAuth.ts` already uses for its own localStorage reads, but additionally catching a corrupted/incompatible stored value the way `useDraftAutosave.ts`'s own JSON.parse read already does). A group whose current route matches one of its children always renders expanded regardless of the persisted flag (`isGroupExpanded()` ORs the two), so landing directly on e.g. `/admin/api-keys` never hides which nav section you're in. Both the desktop `<aside>` and the mobile `<USlideover>` nav lists render groups identically (same duplication precedent as everything else in this file — the two nav lists have always been hand-kept in sync, not componentized).
- **Topbar** (`sticky top-0 z-10` inside `<main>`, **always visible, not desktop-only** as of 2026-09-01 — see below): dark glass to match the sidebar (`bg-(--color-sidebar-bg)/90` + `backdrop-blur-2xl`, same top-left sheen overlay), holding `CrmGlobalSearch` and a page title that fades in (via an `IntersectionObserver` on the page's own `<h2>`) once the user scrolls past it — so the title stays visible without duplicating it permanently. (An earlier iteration of this doc said the topbar had been removed entirely; it was reintroduced since, and this line was stale — always check `layouts/default.vue` directly rather than trusting this doc's memory of it.) **Updated 2026-09-10**: `CrmGlobalSearch` now also focuses on `Cmd+K`/`Ctrl+K` from anywhere on the page (a document-level `keydown` listener, excluding Shift/Alt so it doesn't also swallow e.g. Ctrl+Shift+K), not mouse-only as before — a `<kbd>` hint (`⌘K` on Mac, `Ctrl+K` elsewhere, detected off `navigator.platform`/`userAgent`) shows in the input's trailing slot while it's empty.
  - **Sidebar brand row / topbar height must match exactly** — their border-bottom lines sit side by side at the top of the app shell and any mismatch reads as a broken seam. Both are pinned to the shared `--layout-header-height` token (`3.5rem`, in `assets/styles/global.css`) via `h-(--layout-header-height)`. This was a real bug: the topbar once used `py-2.5` (content-driven height) instead of a fixed height, so it silently drifted out of sync with the sidebar's fixed `h-14` whenever the search input's own padding changed. **Never give either row a padding-driven height (`py-*` alone) — always set `h-(--layout-header-height)` explicitly**, and if this value ever needs to change, change the token once rather than editing both `h-14` occurrences by hand.
- **Mobile — fixed 2026-09-01**: the mobile menu trigger used to be a `fixed top-3 left-3 z-20 md:hidden` button floating *on top of* whatever a page rendered at its own top-left — on a page whose `<h2>` heading sat flush there (e.g. the Dashboard's "Sales Pipeline Dashboard" title), the button visually overlapped the text. The topbar above was `hidden md:flex` at the time, so mobile had no header row at all for the button to live in. Fixed by making the topbar always-visible (never `hidden`) and moving the menu trigger in-flow into it (`md:hidden` on the button itself, not the bar), with the duplicate page-title `<p>` now `hidden md:block` instead (no room for menu button + search + title together on a narrow screen, and the page's own on-page `<h2>` already shows the same title). `CrmGlobalSearch` stays visible at both sizes. The trigger opens a `USlideover` drawer that mirrors the sidebar's nav list, brand header, and footer (lang switch + logout) via named slots (`#header`, `#body`, `#footer`) — note the drawer's own footer renders on a light background (Nuxt UI's default `USlideover` panel), unlike the dark sidebar, so its danger (logout) button uses light-theme colors (`text-red-500 hover:bg-red-50`) rather than the sidebar footer's dark-theme ones (`text-red-300/80 hover:bg-red-500/15`) — don't copy one's color classes onto the other.
- **Main content**: `<main class="flex-1 overflow-y-auto bg-[var(--color-content-bg)]/60">` — see §2.5.
- **`scrollbar-hide` utility (added 2026-09-14)**: the sidebar nav list's `overflow-y-auto` wrapper (line 158's nav list, once the growing menu exceeds viewport height) showed a native scrollbar cut into the dark glass panel. Fixed with a custom Tailwind v4 `@utility scrollbar-hide` (`assets/styles/global.css`) that sets `scrollbar-width: none` + `-ms-overflow-style: none` + `&::-webkit-scrollbar { display: none }` — hides the browser-drawn scrollbar cross-browser while the element stays fully scrollable (wheel/touch/keyboard). Also applied to the app's horizontally-scrolling `UTabs` strips on narrow viewports (`RoleTabs`, Deal/Company detail tabs, pipeline-config, trash) and the Projects/Products kanban board (`PipelineBoard`), which had the same look-and-feel need and previously duplicated the same three bracket-notation utilities inline. **Only for chrome-like scroll containers** (nav panels, tab strips, kanban boards) where the scrollbar reads as clutter, not for scrollable *data* (tables, long lists, modal bodies per §modal.md) — there the scrollbar is a useful signal that more content exists, so leave it visible.
...
`pages/login.vue` has a much more elaborate glass/3D-glass visual treatment (custom SVG "GG" glass logo via `<mask>`+gradients, animated-looking circuit-trace SVG lines, a `backdrop-blur-sm backdrop-saturate-50 backdrop-brightness-110` card) than the rest of the app. **This is deliberate and scoped to the login screen only** — the main authenticated app (sidebar/topbar/content) uses a much lighter touch: sidebar/topbar glass is `backdrop-blur-2xl` over a plain dark fill, no SVG logo decorations, no circuit traces. Don't port the login page's full glass treatment (the SVG logo, circuit traces, saturate/brightness tuning) into internal app pages; it was tuned specifically for a marketing-style entry screen, not a daily-use dashboard.

List-page filter cards get their own, even lighter glass touch — `constants/ui.ts`'s `GLASS_PANEL_UI` (`bg-white/65 backdrop-blur-2xl border border-white/90`), applied via `:ui="GLASS_PANEL_UI"` to the filter `UCard` on every `pages/crm/*/index.vue` / `pages/admin/*/index.vue` page. This is intentional and centralized (not a login-treatment leak) specifically so all 7+ call sites can't drift out of sync — use this constant for any new list page's filter card rather than a one-off class string or a plain `UCard`.
...
Search/filter refs on these pages use `useQuerySyncedRef(key, defaultValue, debounceMs?)` (`composables/utils/useQuerySyncedRef.ts`), not a plain `ref('all')` — it seeds from `route.query` at setup (same deep-link behavior `useQueryFilter` already had) and also writes every later change back via `router.replace`, so a filter/search picked by hand survives a back-button return to the list instead of only an initial deep link. Pass the page's own debounce delay (typically 400, matching `refetchDebounced`) as the third argument for the free-text search field so the URL doesn't rewrite on every keystroke; filter dropdowns pass no third argument (write immediately). Use `useQueryFilter` only for a param this page never writes back (rare — every current list page's filters are user-editable, so they all use `useQuerySyncedRef`).
...
Same title+`ContainerTemplate`+`Form` shell as §5.5, but title is dynamic (e.g. `{{ lead.name }}`) and usually sits inside a header row with a back button and status badge (see `pages/crm/leads/[id].vue`, `pages/crm/deals/[id].vue`, `pages/crm/companies/[id].vue`, `pages/crm/contacts/[id].vue`, `pages/admin/users/[id].vue`). Detail pages for entities with sub-sections (deals, companies) use `UTabs` inside the card for Overview/Activity/etc. Every detail page's back button is the same icon-only `UButton` (`icon="material-symbols:arrow-back"`, `variant="ghost"`, `color="neutral"`, `class="cursor-pointer p-0 hover:bg-transparent"`) with `:aria-label="t('global.back')"` — that shared locale key exists specifically so every back button gets a screen-reader-accessible name without each page inventing its own copy. `pages/admin/users/[id].vue` used to be the one exception (a `UBreadcrumb` with no back button) — it was brought in line with this pattern; don't reintroduce a breadcrumb-only header for a new detail page.
...
|---|---|---|
| `ButtonPrimary` (`components/Button/Primary.vue`) | The only button wrapper in the app | Wraps `UButton`. Props: `outline` (→ `variant="outline"`), `flat` (→ `variant="ghost"`), `block`, `fitContent`, `small`, `color` (default `primary`). Always `rounded-full px-6 min-w-24`. **There is no separate `ButtonOutline` component** (despite older docs implying one) — use `<ButtonPrimary outline>`. The `cancel` shorthand prop (used for the standard dismissive "Cancel" button next to every submit/confirm button) renders **neutral** outline, not red/`error` — a Cancel button is a non-destructive dismissal and shouldn't visually compete with an actually-destructive confirm button (e.g. `CrmConfirmDeleteModal`'s Delete, which stays `error`-colored via its own `confirmColor` prop). Don't reintroduce a red `cancel` — that previously made every Cancel button in the app (including plain non-destructive form Cancels) look as alarming as an actual delete. |
...
| `TableData` / `TablePagination` / `Table/Card/*` (`components/Table/`) | The data-table system, paired with `TABLE_CARD_TYPE` (`constants/tableCardType.ts`) for column render types (STATUS, ACTION, LINK, MULTI_LINE, UPDATED_AT) | Every list page uses this instead of a hand-rolled table. The Action column is always centered (header + cells) and defaults to a fixed 110px width (`ACTION_COLUMN_WIDTH` in `Table/Data.vue`) when a column doesn't set its own `width` — don't hardcode a per-page Action column width (100/120/etc.) anymore, let it fall through to the shared default so every list page's meatball-menu column matches. An action's `isBorderBottom: true` flag renders a divider after it in the dropdown menu (via Nuxt UI's grouped-items array-of-arrays), used to separate safe actions (View Detail/Edit) from a destructive one (Delete/Deactivate) that follows — set it on the action immediately before the destructive one, not on the destructive one itself. The desktop table shows a `global.loading` row while `loading` is true (mirroring the mobile view), instead of silently rendering stale/empty rows with no feedback. **Fixed 2026-09-10**: a sortable column's header icon now reflects which column is active and its direction (`unfold-more` when inactive, `arrow-upward`/`arrow-downward` on the active one) — it used to render the same static `unfold-more` glyph regardless of sort state, silently not doing what §6 of `table.md` already called for. Clicking a *different* sortable column now also always starts that column at `asc` (previously the shared direction toggled unconditionally on every click, so switching columns could just as easily land on `desc` first). |
...
| `CrmMetricBar` | Horizontal progress/amount bar (label + track + fill + slot for trailing stats) | Default fill `bg-sky-400` — see §2.6, don't reconnect to primary. Optional `tooltip` prop (falls back to `"label — percent%"`) wraps the track in a `UTooltip`. Optional `to` prop (added 2026-09-03) turns the whole row into a `NuxtLink` with a hover background, for dashboard bars that deep-link into a filtered list view — see the deep-linking note below. |
| `CrmStatCard` | Dashboard/report stat tile (`UCard` + label/value/hint) | `valueClass`/`hintClass` props for conditional coloring (e.g. green when healthy, red when not). Optional `icon`/`iconClass`/`iconBgClass` render a colored icon chip; optional `accentGlassClass` renders a frosted-glass gradient strip down the card's left ~10% (the root `UCard` has `overflow-hidden` specifically so this strip is clipped to the rounded corners). When a page shows several same-shaped stat cards side by side (lead-source conversion, pipeline stages, industries), assign the icon/bar colors from `constants/ui.ts`'s `CHART_CATEGORICAL_COLORS`/`CHART_FALLBACK_COLOR` by index — don't hand-roll a second copy of that palette, and don't cycle past its 4 entries (5th+ item gets the fallback gray). Deliberately kept compact: root `UCard` forces `:ui="{ body: 'p-3' }"` (no `sm:p-4` bump), icon chip is `size-8`/`size-4`, value is `text-xl`, label/hint are `text-xs`/`text-[11px]` — an explicit call at `pages/index.vue`'s "Pipeline Overview" section (9 stat cards on one dashboard) to trade a denser look for less scroll height; don't revert to the airier `text-2xl`/`size-9` sizing to "match Nuxt UI defaults" without re-checking that tradeoff still holds. Optional `to` prop (added 2026-09-03), same as `CrmMetricBar`'s — wraps the card in a `NuxtLink` with a `hover:shadow-md`, for a stat that deep-links into its underlying list. **`reserveHintSpace` prop (added 2026-09-10, opt-in, default `false`)**: without it, the `#hint` slot's line only renders — and only takes up space — when a card actually has hint content, so a hint-less card (e.g. Avg Sales Cycle) sits visibly shorter than a hint-bearing sibling (e.g. Pipeline Coverage) in the same grid row. Set it on **every** card in a grid where at least one sibling ever uses `#hint` and at least one doesn't, so they all reserve that line's height and stay level — `pages/index.vue`'s "Pipeline Overview" 9-card grid is the one place this applies today (3 of the 9 use `#hint`). Don't set it on a grid where no card ever has a hint (every other `CrmStatCard` caller — report pages, Marketing/Production dashboard summaries) — that would just add dead blank space to every card there for no layout benefit. |
| `CrmSourceBreakdownReport` (added 2026-09-15) | Shared body for a "conversion by source" report page — empty-state alert, an overall-totals summary row (total/converted/overall-rate `CrmStatCard`s), a ranked `CrmStatCard` grid, and a ranked `CrmMetricBar` list, all driven off one `rows: { source, total, converted, conversion_rate }[]` prop | Extracted from `pages/crm/reports/lead-source.vue` and `prospect-source.vue`, which were previously two near-identical hand-copied templates. Every card/bar gets a `to="${linkBase}?source=<row.source>"` deep link into the underlying Lead/Prospect list (see the deep-linking note below). Labels (`totalLabel`, `convertedLabel`, etc.) are passed pre-translated by the caller — the component itself imports no i18n — since "converted" means "Qualified" for Leads and "Converted to Lead" for Prospects; a caller whose source rows use a different field name for the count (`LeadSourceConversionRow.qualified`) maps them into the shared `{ converted }` shape locally rather than this component special-casing field names. **Ranking (revised 2026-09-16)**: rows sort by `total` descending, `conversion_rate` only as a tie-break — sorting by raw `conversion_rate` first let a 1/1 (100%) source outrank a 400/500 (80%) one, which is backwards for "which source matters most." **Loading states (added 2026-09-16)**: a first load (no rows yet) renders a `USkeleton` stand-in for the whole layout instead of flashing zeroed summary cards and an empty grid/list; a refetch with rows already on screen (a filter changed) instead dims the existing content (`opacity-50 pointer-events-none`) rather than replacing it, so the view doesn't jump. Both report pages also disable their "Export CSV" button while `rows.length === 0`. |

**Dashboard card deep-linking (added 2026-09-03; extended 2026-09-15 to the Lead/Prospect Source report pages themselves, not just their dashboard summaries).** Any stat card/breakdown bar whose value maps to an existing list-page filter should pass `to` so a viewer can click straight into the filtered records, rather than the number being a dead end. Wired so far: Marketing dashboard's Total/Open Prospects cards → `/crm/prospects` (unfiltered — "open" spans multiple statuses, no single-value filter fits), Conversion Rate card → the Prospect Source report; its status/source breakdown bars → `/crm/prospects?status=`/`?source=` (Prospects' list view already reads these via its existing `statusFilter`/`sourceFilter` refs, seeded from `route.query` at setup — see `pages/crm/prospects/index.vue`). Sales dashboard's Pipeline-by-Stage bars → `/crm/deals?stage=` (this *added* a `stageFilter` to the Deals list/table — it didn't exist before); Team Performance rows → `/crm/deals?assigned_to=`. `pages/crm/reports/lead-source.vue`/`prospect-source.vue` themselves (via `CrmSourceBreakdownReport`) → `/crm/leads?source=`/`/crm/prospects?source=`, same seeded-from-`route.query` filters. Deals' assignee/business-unit/channel/stage filters are all seeded from `route.query` the same way, forcing List view when any is present (Kanban shows every stage/lane side by side, so a single-stage or single-rep deep link reads better landing on the unambiguous List view — see `hasDeepLinkFilter` in `pages/crm/deals/index.vue`). Deliberately left unwired: Win Rate by Industry and the Sales Funnel/Outcome Split charts — Deals has no industry or won/lost-stage filter today, so these would need new filter UI first, not just a link; don't add a `to` to them without building that filter. Two small shared composables back this: `useOptionalLink(toRef(props, 'to'))` (`composables/utils/useOptionalLink.ts`) gives `CrmStatCard`/`CrmMetricBar` their `linkTag`/`linkTo` — don't hand-roll a third `:is="to ? 'NuxtLink' : 'div'"` inline; and `useQueryFilter(route.query, key)` (`composables/utils/useQueryFilter.ts`) is the `'all'`-fallback seed every deep-linkable filter ref on Deals/Prospects uses — reach for it instead of repeating the `typeof query[key] === 'string' ? ... : 'all'` ternary when a new filter gains deep-link support.

**Bug found and fixed 2026-09-09 — every card/bar described above was a dead link from the day this feature was added.** `useOptionalLink` returned the bare string `'NuxtLink'` for `linkTag`, and `<component :is="linkTag">` resolving a *string* component name requires Vue to find it in the app's global component registry at render time — that lookup silently failed in this app (no console warning, no error — it just rendered a literal, inert `<nuxtlink>` custom element instead of a real `<a>`). Every stat card/breakdown bar listed above (Total/Open Prospects, Conversion Rate, the status/source breakdown bars, Pipeline-by-Stage, Team Performance) looked identical to a working link — hover states and all, since those come from the wrapping `class`, not the tag — but none of them actually navigated anywhere. Fixed by importing the real component object (`import { NuxtLink } from '#components'`) and returning that directly instead of its name, sidestepping runtime name resolution entirely. No other change needed — same `linkTag`/`linkTo` contract, same call sites. Verified live: 14 previously-dead links on the Marketing dashboard alone now resolve correctly.
...
| `CrmActivityTimeline`, `CrmPipelineBoard`, `CrmTeamMemberSelect`, `CrmConfirmDeleteModal` | Domain-specific composed components | `PipelineBoard` resolves a stage's color by first checking the hand-authored `DEAL_STAGE_COLORS` fallback map (`constants/mockData/deals.ts`, covering only the original default stages), then the Admin-configurable `stores/pipelineStages.ts` (`pages/admin/pipeline-config.vue`), then `FALLBACK_COLOR` — see §10 for the remaining fragility. It also takes an optional `columnCounts` prop (`Record<string, number>`) so a column's header count can reflect a server-side total instead of just `grouped[column].length`, and an optional `column-footer` slot (scoped with `{ column }`) for a per-column "Load more" affordance — both added for `pages/crm/deals/index.vue`'s per-stage-paginated Kanban board (resolves the Kanban-board exception previously noted under `api-system-spec.md`'s NFR-003). **Added 2026-09-10 — mobile fallback**: the side-scrolling drag-and-drop column layout is now `hidden md:flex` (native HTML5 drag has no touch equivalent, so it was previously unusable, not just cramped, below `md`). Below `md` it renders a second, separate template block instead — each stage as a collapsible full-width section (default expanded, per-section `collapsedColumns` Set), and moving a card between stages via a per-card `USelectMenu` (bound to the column's own `value`/`label`, matching the `Select` shape already used for `columns`) that emits the same `move` event drag-and-drop does — so a caller's `@move` handler needs no changes to support both. **Fixed 2026-09-14 — keyboard accessibility**: every card (both the desktop drag-and-drop version and the mobile tap version) was only clickable via a bare `@click` on a plain `<div>`, with no way to reach or activate it from a keyboard. Both now carry `role="button"`, `tabindex="0"`, an `@keydown.enter.space` handler firing the same `select` emit as a click, and a `focus-visible:ring-2 focus-visible:ring-(--color-focus)` ring (the app's shared focus-highlight token, §2.4) so keyboard focus is visible against the white card background. |
| `CrmViewModeToggle` (added 2026-09-14) | The Kanban/List segmented view-switcher pill (`v-model` of `'kanban' \| 'list'`, plus `kanbanLabel`/`listLabel` props for the per-page i18n strings/tooltips) | Extracted from `pages/crm/deals/index.vue` and `pages/crm/prospects/index.vue`, which had copy-pasted the exact same markup (the prospects copy even said so in a comment). Any future Kanban/List page should use this component instead of re-inlining the toggle a third time. |
...
| `CrmAddQuoteModal` | Add/edit a Quote, with a dynamic add-item line list | The line-item list (`v-for="item in items"`) is wrapped in `max-h-[40vh] overflow-y-auto` — a user can add an arbitrary number of items, and without this cap they'd push the modal's header/footer off-screen instead of scrolling internally. Any modal with an unbounded-length field list should follow the same pattern rather than letting the whole `UModal` grow past the viewport. |
...
- **`update(id, changes)` is a full-record `PUT`, not a partial `PATCH`, for most entities — `changes` must carry every field, not just the ones actually changing.** The backend handler (e.g. `prospects.go`'s `Update`, `leads.go`'s `Update`) `BodyParser`s the request straight into a form struct and overwrites every mapped field on the record unconditionally — a field the request body omits doesn't survive as "unchanged", it lands as that field's Go zero value (`""`, `nil`, `0`). Every edit-page `onSave` already resends its form's full current state for exactly this reason (see `pages/crm/prospects/[id].vue`, `pages/crm/leads/[id].vue`). **A small number of stores/endpoints are the deliberate exception** — a genuine partial merge, safe to call with a narrow payload — because their handler explicitly checks each field before applying it (e.g. `contracts.go`'s `Update`: `if form.Status != "" { contract.Status = form.Status }`); `stores/contracts.ts`'s `update()` signature (`{ status?, quote_id? }` instead of `Partial<Omit<Contract, 'id'>>`) reflects that. `stores/deals.ts`'s `updateStage()` sidesteps the question entirely by hitting its own dedicated `PATCH /deals/:id/stage` endpoint, which only ever touches `stage` on the already-loaded record. **Don't assume `update()` is a safe partial merge without checking the specific store/handler pair first** — when in doubt, resend the full record (spread the current item, override just the changed field) the way every edit-page form already does.
- **Bug found and fixed 2026-09-15 — dragging a Prospect or Lead card between Kanban columns blanked out the rest of that record.** `pages/crm/prospects/index.vue`'s and `pages/crm/deals/index.vue`'s (Lead-lane) `onMove` handlers called `prospectsStore.update(id, { status: newStatus })` / `leadsStore.update(id, { status: newStatus })` — a narrow, single-field payload, against the full-overwrite `PUT` semantics described above. The card's name/email/company/assignee/etc. all got wiped to their Go zero values by the very next `fetchAll()`/re-render, reading as the card's content "just disappearing". Fixed by having both `onMove` handlers resend the full current record's fields (mirroring their entity's own detail-page `onSave`), changing only `status`. Deals themselves were never affected — their Kanban move goes through `updateStage()`'s dedicated partial-PATCH endpoint, not the general `update()`.
- **Hardened at the type level the same day**, rather than leaving this to a code-review reminder: `stores/prospects.ts`'s and `stores/leads.ts`'s `update()` used to accept `Partial<Omit<Prospect, 'id'>>`/`Partial<Omit<Lead, 'id' | 'score'>>` — TypeScript happily allowed a single-field `{ status }` object, which is exactly what caused the bug above. They now take `ProspectUpdatePayload`/`LeadUpdatePayload` (`interfaces/crm.d.ts`, defined via `Required<Pick<Prospect/Lead, ...>>`) — every field the backend actually overwrites is required, so a future call site missing one is a compile error instead of a runtime data-loss bug. `LeadUpdatePayload`'s `classification` is the one intentionally-optional field, matching the backend's own "omitted classification means leave it alone" handling. If you add a genuinely new optional field to Prospect/Lead that this endpoint doesn't require resending, extend the payload type's exclusions deliberately — don't widen it back to `Partial<...>` to make a new call site's type error go away.
