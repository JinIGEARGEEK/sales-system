...
| FR-CRM-046 | "New Quote" action from a Deal shall pre-fill Deal value, rather than opening a blank form. | M | ✅ **2026-08-21** `CrmAddQuoteModal` seeds one line item (`description: deal.title, qty: 1, price: deal.value`) when opened with a `:deal` prop (`pages/crm/deals/[id]/quotes.vue`), still fully editable. Company/Contact/currency aren't separately pre-filled since Quote has no such fields of its own — it's Deal-scoped only (`deal_id`), so Company/Contact are already implicit via the parent Deal, not something the Quote form needs to duplicate |
...
| FR-CRM-064 | When a Deal is marked "Won," the system shall automatically create/update Customer-Product records for each Product on the Deal's Quote, setting status to "Active." | M | ⬜ **corrected 2026-08-21** — `internal/handlers/deals.go`'s `UpdateStage` `isWon` case contains only a comment placeholder (`// Hook point: FR-CRM-064 auto-creates/updates a CustomerProduct...`), no actual CustomerProduct-creation code; the previous 🚧 status here was inaccurate. CustomerProduct records today are created manually only (FR-CRM-065). This matches `user-story.md`'s UC-1 status note, not this doc's prior (overstated) gap summary — see that file for the cross-reference. |
