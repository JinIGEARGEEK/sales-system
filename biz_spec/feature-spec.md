...
| FR-CRM-046 | "New Quote" action from a Deal shall pre-fill Deal value, rather than opening a blank form. | M | ✅ **2026-08-21**, updated **2026-08-22** — `CrmAddQuoteModal` pre-fills the Scope of Work field with `deal.title` (project-level narrative) and seeds one line item with `description: deal.title, qty: 1, price: deal.value` when opened with a `:deal` prop (`pages/crm/deals/[id]/quotes.vue`) — the seeded row is immediately savable as-is (the item description field is `required`), still fully editable, and a rep who doesn't want the same title in both places can freely shorten/clear the line item's copy. Company/Contact/currency aren't separately pre-filled since Quote has no such fields of its own — it's Deal-scoped only (`deal_id`), so Company/Contact are already implicit via the parent Deal, not something the Quote form needs to duplicate. |
