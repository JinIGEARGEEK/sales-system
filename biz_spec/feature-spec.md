...
| FR-CRM-004 | System shall support converting a Qualified Lead into a Deal (and Contact/Company if new), either via a manual "Convert to Deal" action or automatically when a Lead card is dragged past "Qualified" on the unified Deals Pipeline board (FR-CRM-022). | M | ✅ both paths now call `POST /leads/:id/convert` (§3, api-system-spec.md) — the manual form used to call plain `POST /deals` instead, which silently never marked the Lead converted; fixed 2026-08-16 |
...
| ID | Requirement | Priority | Status |
|---|---|---|---|
| FR-CRM-040 | System shall support creating a Quote/Proposal with line items, unit price, quantity, discount, and total, attached to a Deal. | M | ✅ **updated 2026-08-23 — discount gap closed.** Following a reference (an accounting-style quotation builder shared by the business owner), `QuoteItem` gained `discount_percent` (per-line, 0-100) and `Quote` gained `discount_total` (a further flat-amount discount across the whole quote), computed via the new `internal/utils/quote_totals.go`'s `ComputeQuoteTotals` — shared verbatim by the PDF export (FR-CRM-042) and the frontend editor so neither can silently disagree with the other. Quote also gained `number` (generated `QT{YYYYMM}{seq}`, race-safe), `reference_number`, `issue_date`, `credit_days`, `price_type` (excl/incl tax), `vat_enabled` (Thailand's fixed 7%), `wht_enabled`/`wht_rate`, `notes`/`internal_notes` — see `api-system-spec.md` §7.4 for the full field list and defaults. Previously: real per-Deal Quote store/API with description/qty/price line items and a computed total, no discount field (see FR-CRM-062 for the line-item-vs-Product gap, still tracked separately). Quote gained a whole-quote `scope_of_work` free-text narrative (deliverables/phases/terms), distinct from each line item's own short `description`; line-item descriptions are now also multi-line (2026-08-22). |
...
| FR-CRM-042 | System shall support exporting a Quote as a PDF using a company template. | S | 🚧 `GET /quotes/:id/export-pdf` generates a real PDF (`github.com/go-pdf/fpdf`) — line items, Deal/Company/Contact header, status, validity date — but it's still a plain unbranded layout, not a company template (no logo/brand assets, per the earlier-declined PDF-branding ask). **Updated 2026-08-22** — now also renders `scope_of_work` as a wrapped paragraph above the line-items table; the shared `RenderLineItemsTable` (also used by Contract's export) now wraps multi-line item descriptions onto properly-sized rows instead of silently clipping them to a fixed single-line cell height. **Updated 2026-08-23** (quotation-builder rebuild) — now also renders `number`, Company address/tax ID (closing a gap where Contract's export had this and Quote's didn't), `reference_number`/`issue_date`/`credit_days`/`price_type`, a per-item Discount % column (`RenderQuoteItemsTable`), and a full totals block (discount/VAT 7%/WHT/grand total via `ComputeQuoteTotals`) plus `notes` as a footer paragraph — still no signature/stamp block (explicitly deferred) and still not visually branded. |
...
| FR-CRM-046 | "New Quote" action from a Deal shall pre-fill Deal value, rather than opening a blank form. | M | ✅ **2026-08-21**, updated **2026-08-23** — `CrmAddQuoteModal` pre-fills the Scope of Work field with `deal.title` (project-level narrative) and seeds one line item with `qty: 1, price: deal.value` when opened with a `:deal` prop (`pages/crm/deals/[id]/quotes.vue`); the line item's own `description` starts blank rather than also copying `deal.title`, since that narrative now lives in Scope of Work — a line item's description is optional (no `required` rule, on this or any other row), so a blank one never blocks Save. Company/Contact/currency aren't separately pre-filled since Quote has no such fields of its own — it's Deal-scoped only (`deal_id`), so Company/Contact are already implicit via the parent Deal, not something the Quote form needs to duplicate. A Sales Rep is expected to replace the single pre-filled row with one line item per feature/service named in the Scope of Work (via "+ เพิ่มรายการ"/"Add Item"), each priced independently — the modal now shows a live running total (`itemsTotal`, qty×price summed across all rows) next to the item list so the rep can check the breakdown reconciles with the intended total before saving, rather than only finding out from the exported PDF's Grand Total after the fact. The section header itself was renamed from the Thai "รายการสินค้า" ("product item list") to the neutral "รายการ" ("Items") since a single quote's rows are a mix of two different things — a bare row (no `product_id`) is a custom scope/feature of a Project, priced by hand; a Product-linked row is one of the company's own packaged offerings, priced from the Catalog — and each row now starts with an explicit "สโคป/ฟีเจอร์" (Scope/Feature) vs "แพ็กเกจราคา" (Pricing Package) `URadioGroup` toggle a rep picks *before* filling the row in, rather than inferring the kind from whether a product happened to be picked — the Product picker itself only renders at all once "Pricing Package" is chosen, so it no longer sits above the description looking like a mandatory first step on every row. Switching a row back to "Scope/Feature" clears its `product_id`. This is per-row state (`item.kind`), not a new field on Deal/Quote — no such "Project vs Product sale" classification exists on either model, and a single quote can still mix both kinds of row. |
