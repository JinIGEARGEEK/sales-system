...
| # | User Story | Refs | Status |
|---|---|---|---|
| S-18 | As a Sales rep, when I mark a Deal "Won," I want the Products on its Quote to automatically appear as "Active" on the Company's product list, so that I don't have to manually re-enter what they bought. | FR-CRM-063, FR-CRM-064 | 🚧 **corrected 2026-08-22** — Product/CustomerProduct tracking itself is built (FR-CRM-063 ✅), but the auto-create-on-Won side effect (FR-CRM-064) is still not implemented (`internal/handlers/deals.go`'s `UpdateStage` `isWon` case is only a comment placeholder) — a rep must add the CustomerProduct manually today (S-19) after a Won Deal, optionally linking it back to that Deal via the new Deal picker in `AddCustomerProductModal.vue`. |
