...
> **Implementation status (as of 2026-08-14, corrected 2026-08-22):** Each story now carries a **Status** column derived from `feature-spec.md` §9's gap summary. Legend: ✅ Built · 🚧 Partial · ⬜ Not built. Section 2.5 (Real Products & Projects Tracking) was previously the biggest gap here but its individual rows (S-18–S-22) were stale — Products/CustomerProducts/Projects are now built end-to-end. S-18's FR-CRM-064 (auto-create-on-Won) is **not a remaining gap** — confirmed with the business owner (2026-08-22) that it will not be built; see S-18's own row for the rationale. §2.4's Quote/Contract rows have not been re-audited in this pass — see `feature-spec.md` §9 for the full picture.
...
| S-2 | As a Sales rep, I want to convert a Qualified Lead into a Deal (creating the Company/Contact if new) with title, value, currency, owner, and expected close date, so that I can start tracking it through the pipeline. | FR-CRM-004, FR-CRM-020 | ✅ manual "Convert to Deal" on Lead Detail now asks for confirmation first, since it's irreversible |
...
| S-12 | As a Sales rep, I want to set follow-up reminders on a Deal (or a Contact/Company) with a priority and description, and edit them later, so that I never miss a next step. | FR-CRM-032 | 🚧 Tasks tab on Deal/Contact/Company detail pages, a dedicated all-tasks page (`/crm/tasks`) with filters and bulk mark-done/reassign, confirm-before-done dialog, and an auto-created follow-up task on Deal Won (`stores/tasks.ts`); due-date notification is now built as email (a 15-min `internal/notifier` ticker) but needs real SMTP credentials configured to actually send; push notification not built. **Updated 2026-08-22** — added a Low/Medium/High `priority` label and a free-text `description` separate from title, and a task can now be edited after creation (title/description/due date/priority/assignee) via a new `PATCH /tasks/:id`, not just toggled done or bulk-reassigned. |
...
| S-14 | As a Sales rep, I want to build a Quote from Products in our catalog, so that pricing stays consistent and line items are never free-text guesses. | FR-CRM-040, FR-CRM-062 | 🚧 a real per-Deal Quote (`stores/quotes.ts`); each line item can now optionally pick a Product from the catalog via `AddQuoteModal.vue`, snapshotting its name/price at save time — free-text items are still allowed too, so this isn't yet mandatory product-only pricing. **Updated 2026-08-22** — for bespoke-project quotes (this business's primary case, not catalog goods), the Quote also carries a whole-quote `scope_of_work` narrative field (deliverables/phases/terms), and each line item's description is now multi-line, both included in the exported PDF. |
...
| S-16 | As a Sales rep, I want to export a Quote as a branded PDF, so that I can send something professional to the client. | FR-CRM-042 | 🚧 **corrected 2026-08-22** — this row was stale. `GET /quotes/:id/export-pdf` is built and wired to a Download-PDF button on the Deal's Quotes tab (`pages/crm/deals/[id]/quotes.vue`); the only real gap is the PDF being a plain unbranded layout rather than a company-branded template. |
| S-17 | As a Sales rep, I want to attach a signed Contract to a Deal, so that there's a single source of truth for what was agreed. | FR-CRM-043, FR-CRM-044 | ✅ **corrected 2026-08-22** — this row was stale; both FR-CRM-043 and FR-CRM-044 are built (Contract CRUD + a Contracts tab on the Deal detail page, `POST /contracts/:id/upload` setting the signed file/date and flipping status to Signed). Also updated same day: the Contract's exported PDF now includes the linked Quote's `scope_of_work` narrative alongside its pricing, so "what was agreed" actually covers scope, not just price. |
| S-17b | As a Sales rep, I want the system to (optionally, per Admin config) block marking a Deal "Won" until its Contract is "Signed," so that we never start delivery on a handshake deal. | FR-CRM-045 | ✅ **built 2026-08-22** — an Admin-only toggle in Pipeline Config's Sales Quota section, off by default; when on, moving a Deal to Won (from the Kanban board, the "Mark Won" button, or the full edit form) is blocked with a clear error unless the Deal already has a Signed Contract. **Updated 2026-09-11** — this was easy to miss (only a generic error after the fact); the Deal Overview and Contracts tabs now show a proactive warning banner when the toggle is on and no Contract is Signed yet, and every place the block can fire shows the specific reason instead of a generic error. |
| S-17c | As a Sales rep, I want to save a Quote's line items/scope/pricing as a reusable template and start a new Quote from it, so that I don't rebuild the same quote from scratch every time. | FR-CRM-116 | ✅ **built 2026-09-11** — "Save as Template" on the Quote editor, "Start from Template" on Create Quote; usable/creatable by any Sales role, not curated by Admin. Delete-only management (no rename/edit) for now. |

### 2.5 Real Products & Projects Tracking (the core new capability)

| # | User Story | Refs | Status |
|---|---|---|---|
| S-18 | As a Sales rep, when I mark a Deal "Won," I want the Products on its Quote to automatically appear as "Active" on the Company's product list, so that I don't have to manually re-enter what they bought. | FR-CRM-063, FR-CRM-064 | ⬜ **superseded 2026-08-22 — this story will not be built as written.** Product/CustomerProduct tracking itself is built (FR-CRM-063 ✅), but confirmed with the business owner that auto-creating a CustomerProduct on every Won Deal (FR-CRM-064) is the wrong behavior, not an unfinished one: `Product` represents the company's own packaged products/subscriptions, sold via Contract and charged once/annually, while most Won Deals are custom software projects (→ `Project`, not `CustomerProduct`). A rep adds the CustomerProduct manually (S-19) only for the deals that are genuinely product/package sales, optionally linking it back to the Deal via the Deal picker in `AddCustomerProductModal.vue`. |
| S-19 | As a Sales rep, I want to manually add or update a Company's Product status (e.g., mark a product "Churned" after a failed renewal), so that the record reflects reality even outside a Deal. | FR-CRM-065 | ✅ **corrected 2026-08-22** — built via `AddCustomerProductModal.vue`/`stores/customerProducts.ts`; also gained `start_date` (settable at creation, defaults to today) and `end_date` (settable in edit mode) on 2026-08-22. |
| S-20 | As a Sales rep, when I win a Deal, I want to be prompted to create a Project record (name, dates), so that we start tracking the real engagement right away. | FR-CRM-068 | ✅ **corrected 2026-08-22** — this row was stale; the "disabled placeholder modal" note no longer applies. The Deal-Won flow (and the Contract-Signed flow, `FR-CRM-048`) both prompt `CrmAddProjectModal` via the shared `useCreateProjectFromDeal` composable, creating a real Project linked to the Deal's `company_id`/`deal_id`. The `name` field is now a creatable combobox suggesting previously-used Project names. **Updated 2026-09-08** — the combobox now always shows a "Create '...'" row alongside suggestions as soon as you type (`InputCombobox` default `create-item="{ when: 'always' }"`), instead of only once no suggestion matched, so it no longer reads as select-only when a Company already has Project names on file. |
| S-21 | As a Sales rep, I want to update a Project's status (Not Started/In Progress/On Hold/Completed/Cancelled) and paste in a link/ID to Production's own tracker, so that I can quickly check delivery status without pinging the Production team. | FR-CRM-067, FR-CRM-069 | ✅ **corrected 2026-08-22** — built via `AddProjectModal.vue`/`stores/projects.ts`; no webhook sync (not required per FR-CRM-069). |
| S-22 | As a Sales rep, I want to see every Project (past and current) and every Product a Company has ever had with us, sorted by most recent, right on their Company page, so that I know their full history before a renewal or upsell call. | FR-CRM-066, FR-CRM-070 | ✅ **corrected 2026-08-22** — built via the Company detail page's Products and Projects tabs. |
...
| M-6 | As a Sales Manager, I want to filter any report by date range, rep, or company tag, so that I can slice data the way I need. | FR-CRM-055 | ✅ **corrected 2026-09-08** — this row was stale (contradicted FR-CRM-055's own ✅ in feature-spec.md); dashboard filters by date range, Sales Rep (`assigned_to`), Company tag, Business Unit (Project/Product), and Channel — all six live in `components/Dashboard/FilterBar.vue`/`pages/index.vue`. |
| M-7 | As a Sales Manager, I want a report of "which customers use Product X" and "which customers have a Project in status Y," so that I can spot cross-sell opportunities or accounts that need attention. | FR-CRM-056 | ✅ **corrected 2026-09-04** — this row previously said ⬜ but was stale; `pages/crm/reports/customer-product-status.vue` (`GET /reports/customers-by-product-status`) has been built since FR-CRM-056 was marked ✅ in feature-spec.md. Still distinct from the dashboard's Business Unit/Project/Product filter (see M-9), which is a lightweight Deal tag, not this real per-customer Product/Project report — don't conflate the two. |
| M-8 | As a Sales Manager, I want to reassign a Deal to a different rep and see the prior owner history, so that I can rebalance workload without losing accountability. | FR-CRM-025 | ✅ reassignment works; the "Owner History" card on the Deal detail page (`pages/crm/deals/[id]/index.vue`) shows prior owners, sourced from `GET /audit-log` filtered to `reassigned`/`bulk_reassigned` actions for that Deal. **Corrected 2026-09-08** — this was gated Admin-only with the stated reason "the audit-log endpoint is Admin-only," which was already stale (the endpoint had opened to Sales Rep/Sales Manager for Deal stage-change history — see FR-CRM-098-adjacent Activities work). Widened to Admin/Sales Manager: `internal/handlers/auditlog.go`'s `List` now gives Sales Manager `stage_changed` + `reassigned`/`bulk_reassigned` for `entity_type=deal` (Sales Rep still gets `stage_changed` only), matching this story at face value — the Sales Manager doing the reassigning can now see the history they just created. `usersStore.fetchAll()` (Admin-only `/users`) is skipped for a Sales Manager viewer to avoid a 403; actor names fall back to `teamMembersStore` (active staff only, via the non-Admin-only `/team-members`). |
...
**Updated 2026-09-23 (FR-CRM-123): Marketing now has full Sales Rep parity on Leads/Deals** (and Quotes/Contracts/Payments under a Deal), so the "no access" rule and the read-only "View Lead" exception below are historical; bulk/export/Trash/Reports stay Admin/Sales Manager only. Its primary job is still the Prospect funnel, and it now also sees where hand-offs go next on the Overview Pipeline page.

Added 2026-09-01. Marketing ~~has no access to Leads/Deals/Quotes/Contracts/Payments~~ — its scope is the Prospect funnel that exists *before* a record is ready to become a Lead (§3.1a in `feature-spec.md`). One narrow exception: once a Prospect converts (P-4), its "View Lead" link on the Prospect detail page still reaches the full Lead detail page read-only — no nav/list access, just that one deep link, so Marketing can see where the hand-off landed. **Fixed 2026-09-09**: this page's Mark SQL/Convert to Deal buttons and Save Changes form used to render for anyone who followed that link too, implying Sales-only actions that weren't Marketing's to take — worse, `PUT`/`POST /leads/:id/convert` had no backend role check at all, so those buttons weren't just misleading, they'd have actually worked if clicked. Both are now hidden for non-SALES_PIPELINE_ROLES viewers and rejected server-side (`Admin`/`Sales Rep`/`Sales Manager` only) — the read-only "where did it land" visibility this exception exists for is unaffected.
...
| P-2 | As someone in Marketing, I want to see my Prospects on their own Kanban board (New/Engaging/Nurturing/Disqualified), separate from the Sales team's Deals/Leads board, so that I can manage my own funnel stages without cluttering or being cluttered by the sales pipeline. | FR-CRM-105 | ✅ `pages/crm/prospects/index.vue`, standalone `CrmPipelineBoard` instance. **Updated 2026-09-09** — a status/source/assignee-filtered deep link (e.g. from the Dashboard's Marketing tab breakdown rows) used to force List view, since the status filter pill was List-only and the board had no visible controls for its filters — a bigger relative loss for Marketing than for Sales Rep, since Kanban is Marketing's *only* primary view (Sales Rep also has List as an alternative on the Deals board). The status filter is now shown in both views, so a deep link stays on Kanban with the filter already applied instead of losing the board entirely. Also fixed the same day: `GET /admin/prospect-sources` was Admin-only, so this page's own source dropdown/filter — Marketing's own primary page — 403'd and got redirected away from itself; see api-system-spec.md §8.5's note on the wider sweep. **Updated 2026-09-09 (later)** — the four column names listed above are now the *default seed*, not a fixed set: an Admin can rename/reorder/add/deactivate them from "Prospect Stages & Sources" on `pages/admin/pipeline-config.vue` (`ProspectStage`, `/admin/prospect-stages`, api-system-spec.md §8.7), the same `PipelineStage` mechanism Deal stages already use. The board's columns and the status filter's options now come from that config instead of a hardcoded list. |
...
| P-4 | As someone in Marketing, when a Prospect is sales-ready, I want to convert it into a Lead in one action (reusing or creating its Company/Contact), so that Sales picks up exactly where I left off instead of the record being re-entered from scratch. | FR-CRM-106 | ✅ `POST /prospects/:id/convert`, mirrors `POST /leads/:id/convert`'s Company/Contact resolution and Attachment carry-over one funnel stage earlier. "Convert to Lead" on Prospect Detail now confirms via modal first, since it's irreversible |
...
| # | User Story | Refs | Status |
|---|---|---|---|
| PT-1 | As someone on the Production team, I want to update the status and reference link on a customer's Project record (manually today, possibly via a simple integration later), so that Sales can see accurate delivery status without me repeating it in every account call. | FR-CRM-069 | ✅ **corrected 2026-09-09** — this row was stale; the Production nav-scope work (commit `961a561`) already built it. "Projects" is an unrestricted nav item (`layouts/default.vue`, no `roles` filter) reaching `pages/crm/projects/index.vue`'s row Edit action, which opens `CrmAddProjectModal` in a dedicated `productionEditor` mode (`hasRole('Production') && !!props.project`) showing only Status and Production Reference — every other field (name, dates, notes, company/deal links) is hidden, not just disabled. Enforced server-side too, not just hidden client-side: `PATCH /projects/:id` inspects the raw request body's keys and 403s if a Production caller sends anything beyond `status`/`production_reference` (`internal/handlers/projects.go`'s `productionAllowedKeys`). **Updated 2026-09-09 (later same day)** — Production's Dashboard (`pages/index.vue`) previously had no content tailored to this role at all (neither SALES_PIPELINE_ROLES nor PROSPECT_ROLES, so the entire page rendered empty for them). Added `components/Dashboard/ProductionSummary.vue`: open/not-started Project counts plus a "Projects Needing a Status Update" list (sorted oldest-created first, Project has no `updated_at` to sort by true staleness), each row deep-linking via a new `?edit=<id>` query param on `pages/crm/projects/index.vue` straight into that Project's edit modal — Projects have no per-record detail route, only the list + modal, so a raw id needed this to resolve. No integration exists (still manual, as the story itself allows). |
...
> **Status (corrected 2026-08-21, superseded 2026-08-22):** Product Catalog, Quote, Contract, CustomerProduct, and Project are all now built (see §9 of `feature-spec.md`), so UC-1 steps 1–5 and 7–9 and all of UC-2/UC-3 are walkable today. UC-1 step 6 (below) originally described an auto-CustomerProduct-on-Won hook — confirmed with the business owner (2026-08-22) that this will **not** be built: `Product`/`CustomerProduct` model the company's own packaged products/subscriptions (sold via Contract, charged once/annually, tracked manually per FR-CRM-065), a largely separate population of deals from the custom-software-project deals this UC otherwise walks through (which become a `Project` via Quote/Contract instead, not a CustomerProduct). Step 6 below is kept for narrative completeness but describes a manual action, not an automatic one.
...
6. Rep manually adds a Customer-Product record on Acme's Company page for the packaged product actually sold alongside the custom build — e.g. "Annual Support Plan — Active" — linking it back to this Deal via the Deal picker. (The custom software build itself isn't a Customer-Product at all — it becomes the Project created in step 7 below.)
