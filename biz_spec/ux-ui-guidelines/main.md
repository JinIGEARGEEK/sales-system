...
- Overview boards (read-first): สำหรับหน้าภาพรวมที่ใช้ "อ่าน" มากกว่า "ย้ายการ์ด" เช่น Overview Pipeline (FR-CRM-123)
  ...
  - ช่วยการรีวิว: ใช้การ "ไฮไลต์" (ทำให้การ์ดที่ไม่ตรงเงื่อนไขจางลง) แทนการซ่อน เพื่อคงรูปทรงของบอร์ดไว้; แสดงแบนเนอร์เตือนเมื่อมีรายการค้างที่มีมูลค่า พร้อมปุ่มไฮไลต์ในคลิกเดียว; มีปุ่ม "ไปที่" สำหรับโซนที่อยู่นอกจอ
  - หน้าตาของ lane/การ์ดต้องเหมือนบอร์ด Kanban ของแต่ละ entity: ใช้ `usePipelineStageColors()` ร่วมกัน (สีหัวคอลัมน์ตามขั้นตอน + คำอธิบาย + ตัวเลขจำนวน, พื้นหลังโปร่งแสงสีเดียวกัน) และโครงการ์ดเดียวกัน (ชื่อ, บริษัท, มูลค่า, footer ผู้รับผิดชอบ) เพื่อให้ขั้นตอนเดียวกันดูเหมือนกันทุกหน้า
  - หมวดที่คำนวณจากเวลา (เช่น "ค้าง", "ขยับ") ต้องมี tooltip อธิบายเกณฑ์: นับจากวันไหน เกณฑ์กี่วัน และช่วงเวลาที่ใช้ (แสดงวันที่จริง) ส่วนบนการ์ดให้ tooltip แสดงวันที่ของรายการนั้นเอง; ใช้ข้อความชุดเดียวกันทั้งใน tooltip และ legend; tooltip ที่เป็นประโยคให้ใช้ `MULTILINE_TOOLTIP_UI` (`constants/ui.ts`) เพราะ UTooltip ปกติตัดข้อความเหลือบรรทัดเดียว
  - ห้ามให้รายการหายไปจากบอร์ดแบบเงียบ ๆ: รายการที่ขั้นตอนไม่ตรงกับคอลัมน์ใด (ว่าง/ถูกปิดใช้งาน/ถูกเปลี่ยนชื่อ) ต้องไปอยู่ในคอลัมน์ "ขั้นตอนอื่น" (สีเทา แสดงเฉพาะเมื่อมีรายการ) พร้อมบอกขั้นตอนจริงบนการ์ด และให้เลือกขั้นตอนใหม่ได้จาก side panel
  - ชื่อขั้นตอนเดียวกันของคนละ entity (เช่น "New" ของ Prospect กับ Lead) ต้องระบุ `entity` ให้ `usePipelineStageColors()` เพื่อให้สี/คำอธิบายเป็นของ entity นั้นเอง
  - ตัวเลขสรุป (เช่น จำนวนที่ค้าง/ขยับ) ต้องเป็นยอดจริงจาก API ไม่ใช่นับจากการ์ดที่โหลดมา (ซึ่งมีจำกัดต่อคอลัมน์); อัตรา conversion ต้องเป็นแบบ cohort (ไม่เกิน 100%) และการขยับต้องบอกทิศทาง (เดินหน้า/ถอยหลัง)
  - คำอธิบายสัญลักษณ์ (legend) ที่ยาว ให้ย้ายไปไว้ใน popover "วิธีอ่าน" แทนการวางเป็นแถวข้อความบนหน้า
  - แบนเนอร์เตือนใช้ token ของแอปเอง (`--color-warning-bg` + ขอบซ้าย `--color-warning-hover`) แทน `UAlert` warning/subtle ซึ่งแสดงผลคอนทราสต์ต่ำในธีมนี้
- Content Display
