...
**Keyboard (Implementation จริงในระบบนี้)**
`CrmConfirmDeleteModal` (`components/Crm/ConfirmDeleteModal.vue`) จะ autofocus ปุ่ม confirm ทุกครั้งที่ modal เปิดขึ้น เพื่อให้ผู้ใช้กด Enter ยืนยันได้ทันทีโดยไม่ต้องใช้เมาส์ — สอดคล้องกับพฤติกรรม native dialog ทั่วไปที่ Enter จะ trigger ปุ่มที่ focus อยู่
