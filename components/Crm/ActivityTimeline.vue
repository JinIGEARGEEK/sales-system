<template>
  <div>
    <div v-if="items.length === 0" class="py-6 text-center text-sm text-(--color-gray)">
    ...
    <div v-else class="flex flex-col gap-4">
      <div v-for="item in sortedItems" :key="item.id" class="flex gap-3">
        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-(--color-primary-bg)">
          <UIcon :name="iconFor(item.type)" class="size-4 text-(--color-primary)" />
        </div>
        <div class="min-w-0 flex-1 border-b border-(--color-light-gray-2) pb-3 last:border-none">
          <div class="flex items-center justify-between gap-2">
            ...
            <span class="shrink-0 text-xs text-(--color-gray)">{{ dateTimeFormat(item.created_at.toISOString()) }}</span>
          </div>
          <p v-if="item.notes" class="mt-1 text-xs text-(--color-gray)">{{ item.notes }}</p>
          <p class="mt-1 text-xs text-(--color-gray)">{{ t('crm.components.activityTimeline.by') }} {{ item.created_by }}</p>
        </div>
      </div>
    </div>
  </div>
