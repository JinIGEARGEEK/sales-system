<template>
  <UModal :open="open" @update:open="onUpdateOpen">
    <template #header>
      <h3 class="text-lg font-medium">{{ size ? t('admin.pipelineConfig.companySizes.editTitle') : t('admin.pipelineConfig.companySizes.addTitle') }}</h3>
    </template>
    <template #body>
      <Form ref="formRef" @submit="onSubmit">
        <div class="grid grid-cols-1 gap-3">
          <InputText v-model="form.name" :label="t('admin.pipelineConfig.companySizes.name')" name="name" rules="required" />
...
const props = defineProps<{
  open: boolean
  // Passing an existing CompanySizeOption switches this into edit mode.
  size?: CompanySizeOption | null
}>()
