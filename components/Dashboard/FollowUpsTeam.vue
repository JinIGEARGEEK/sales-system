  ...
  <div>
    ...
    <div class="mb-6">
      <UCard class="ring-[var(--color-card-border)]">
        ...
        <div v-if="upcomingTasks.length === 0" class="py-6 text-center text-sm text-(--color-gray)">
    ...
    <div v-if="canViewSalesPipelineWidgets" class="mb-6">
      <UCard class="ring-[var(--color-card-border)]">
        ...
        <div v-if="recentAlerts.length === 0" class="py-6 text-center text-sm text-(--color-gray)">
    ...
    <div v-if="canViewSalesPipelineWidgets" class="grid grid-cols-1 items-stretch gap-4 lg:grid-cols-5">
      <div class="lg:col-span-3">
        <UCard class="h-full ring-[var(--color-card-border)]" :ui="{ root: 'flex h-full flex-col', body: 'flex-1' }">
          ...
          <div v-if="industryBreakdown.length === 0" class="py-6 text-center text-sm text-(--color-gray)">
          ...
          <div v-else class="flex flex-col gap-3">
            ...
            >
              <span class="min-w-14 shrink-0 whitespace-nowrap text-right text-sm text-(--color-gray)">{{ row.winRate }}%</span>
              <span class="min-w-16 shrink-0 whitespace-nowrap text-right text-xs text-(--color-gray)">{{ t('crm.dashboard.dealsWon', { count: row.wonCount }) }}</span>
            </CrmMetricBar>
          </div>
        </UCard>
      </div>
        ...
        <UCard class="h-full ring-[var(--color-card-border)]" :ui="{ root: 'flex h-full flex-col', body: 'flex-1' }">
          ...
          <div class="flex flex-col gap-2">
            <NuxtLink
              ...
              class="flex items-center justify-between rounded-lg border border-(--color-light-gray-2) px-4 py-3 transition-colors hover:bg-(--color-light-gray-1)"
            >
