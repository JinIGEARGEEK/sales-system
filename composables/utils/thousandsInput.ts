...
// thousands separators, one decimal point and up to 2 fraction digits.
...
export const sanitizeThousandsInput = (raw: string): string => {
  const cleaned = raw.replace(/[^0-9.]/g, '')
  const dot = cleaned.indexOf('.')
  if (dot === -1) return cleaned.replace(/^0+(?=\d)/, '')
  const intPart = cleaned.slice(0, dot).replace(/^0+(?=\d)/, '')
  const fraction = cleaned.slice(dot + 1).replace(/\./g, '').slice(0, MAX_FRACTION_DIGITS)
...
export const formatThousands = (value: string | number | null | undefined): string => {
  if (value === '' || value === null || value === undefined) return ''
  if (typeof value === 'number') {
    return Number.isNaN(value) ? '' : value.toLocaleString('en-US', { maximumFractionDigits: MAX_FRACTION_DIGITS })
  }
  const canonical = sanitizeThousandsInput(value)
  if (!canonical) return ''
