export default {
  index: {
    emptyTitle: 'No deals yet',
    emptyDescription: 'Deals track each sales opportunity from first conversation to won or lost. Add one, or convert a qualified Lead.',
    pageTitle: 'Deals Pipeline',
    heading: 'Deals Pipeline',
    addDeal: 'Add Deal',
    exportCsv: 'Export CSV',
    manageStages: 'Manage Pipeline Stages',
    dealMovedTo: 'Deal moved to {stage}',
    searchPlaceholder: 'Search by title or company...',
    assigneePlaceholder: 'Assigned To',
    disqualifiedLeadBadge: 'Disqualified Lead',
    leadConvertedToDeal: 'Lead converted to a Deal',
    leadStatusUpdated: 'Lead moved to {status}',
    viewKanban: 'Kanban',
    viewList: 'List',
    entityLabel: 'deals',
    loadMoreDeals: 'Load more ({count} more)',
  },
  table: {
    searchPlaceholder: 'Search by title or company...',
    columns: {
      title: 'Title',
      company: 'Company',
      value: 'Value',
      stage: 'Stage',
      assignedTo: 'Assigned To',
      created: 'Created',
      action: 'Action',
    },
    actions: {
      viewDetail: 'View Detail',
      edit: 'Edit',
      delete: 'Delete',
    },
  },
  create: {
    pageTitle: 'Add Deal',
    heading: 'Add Deal',
    subheading: 'Create a new sales opportunity.',
    dealTitle: 'Deal Title',
    dealTitlePlaceholder: 'Enter deal title',
    company: 'Company',
    companyPlaceholder: 'Search or type a new company name',
    primaryContact: 'Primary Contact',
    primaryContactPlaceholder: 'Select contact',
    dealValue: 'Deal Value (THB)',
    dealValuePlaceholder: 'Enter deal value',
    stage: 'Stage',
    stagePlaceholder: 'Select stage',
    forecastCategory: 'Forecast Category',
    expectedCloseDate: 'Expected Close Date',
    businessUnit: 'Business Unit',
    businessUnitPlaceholder: 'Select business unit (optional)',
    project: 'Project',
    product: 'Product',
    businessUnitItemPlaceholder: 'Select which one',
    createDeal: 'Save Deal',
    cancel: 'Cancel',
    createSuccess: 'Deal created',
    duplicateWarningTitle: 'Possible duplicate deal',
    duplicateWarningBody: 'This company already has an open deal:',
  },
  detail: {
    pageTitle: 'Deal Detail',
    markWon: 'Mark Won',
    tabs: {
      overview: 'Overview',
      quotes: 'Quotes',
      contracts: 'Contracts',
      payments: 'Payments',
      tasks: 'Tasks',
      activity: 'Activity',
      attachments: 'Attachments',
    },
    dealTitle: 'Deal Title',
    dealValue: 'Deal Value (THB)',
    stage: 'Stage',
    probability: 'Win Probability (%)',
    forecastCategory: 'Forecast Category',
    lostReason: 'Lost Reason',
    lostReasonPlaceholder: 'Select a reason',
    contractRequiredWarning: 'A signed contract is required before this deal can be marked Won.',
    contractRequiredToast: 'Add a signed contract before marking this deal Won.',
    expectedCloseDate: 'Expected Close Date',
    businessUnit: 'Business Unit',
    businessUnitPlaceholder: 'Select business unit (optional)',
    businessUnitItemPlaceholder: 'Select which one',
    product: 'Product',
    saveChanges: 'Save Changes',
    linkedRecords: 'Linked Records',
    company: 'Company',
    contact: 'Contact',
    project: 'Project',
    projectNotCreatedYet: 'Not created yet',
    ownerHistory: 'Owner History',
    ownerHistoryReassigned: 'Reassigned from {from} to {to}',
    ownerHistoryAssigned: 'Assigned to {to}',
    ownerHistoryBy: 'by {actor}',
    noOwnerHistory: 'No reassignments yet.',
    quotesTitle: 'Quotes',
    createQuote: 'Create Quote',
    createQuoteSuccess: 'Quote created',
    noQuotes: 'No quotes created for this deal yet.',
    validUntil: 'Valid until {date}',
    uploadQuotation: 'Upload Quotation (PDF)',
    invalidFileType: 'Please upload a PDF file.',
    fileTooLarge: 'File is too large (max 10 MB).',
    uploadSuccess: 'Quotation uploaded',
    uploadedOn: 'Uploaded {date}',
    viewPdf: 'View PDF',
    editQuote: 'Edit Quote',
    downloadPdf: 'Download as PDF',
    removeQuotation: 'Delete quotation',
    removeQuotationConfirmBody: 'Delete "{name}"? This cannot be undone.',
    removeQuotationSuccess: 'Quotation deleted',
    updateQuoteStatusSuccess: 'Quote status updated',
    confirmQuoteStatusTitle: 'Change quote status?',
    confirmQuoteStatusBody: 'Mark this quote as "{status}"? This records the customer\'s decision and is hard to reverse.',
    confirmQuoteStatusConfirm: 'Change status',
    paymentsTitle: 'Payments',
    addPayment: 'Add Payment',
    noPayments: 'No payments recorded for this deal yet.',
    totalPaid: 'Total Paid',
    remainingBalance: 'Remaining Balance',
    fullyPaid: 'Fully Paid',
    columnDate: 'Date',
    columnAmount: 'Amount',
    columnMethod: 'Method',
    columnNote: 'Note',
    removePayment: 'Delete payment',
    removePaymentConfirmBody: 'Delete the {amount} payment from {date}? This cannot be undone.',
    addPaymentSuccess: 'Payment recorded',
    removePaymentSuccess: 'Payment deleted',
    paymentScheduleTitle: 'Payment Schedule',
    addInstallment: 'Add Installment',
    noInstallments: 'No payment schedule defined for this deal yet.',
    columnDueDate: 'Due Date',
    columnStatus: 'Status',
    installmentStatus: {
      paid: 'Paid',
      partial: 'Partial',
      overdue: 'Overdue',
      upcoming: 'Upcoming',
    },
    removeInstallment: 'Delete installment',
    removeInstallmentConfirmBody: 'Delete the {amount} installment due {date}? This cannot be undone.',
    addInstallmentSuccess: 'Installment added',
    removeInstallmentSuccess: 'Installment deleted',
    generateSchedule: 'Generate Schedule',
    generateScheduleSuccess: '{count} installments generated',
    tasksTitle: 'Tasks',
    addTask: 'Add Task',
    addTaskSuccess: 'Task added',
    editTaskSuccess: 'Task updated',
    activityTitle: 'Activity',
    addActivity: 'Log Activity',
    addActivitySuccess: 'Activity logged',
    stageHistoryTitle: 'Pipeline History',
    stageHistoryBy: 'by {actor}',
    attachmentsTitle: 'Attachments',
    addAttachment: 'Add Attachment',
    addAttachmentSuccess: 'Attachment added',
    removeAttachmentSuccess: 'Attachment deleted',
    dealNotFound: 'Deal not found.',
    createProjectModalTitle: 'Create Project from this Deal?',
    createProjectModalBody: 'This will create a new Project for the Company, linked back to this Deal.',
    createProjectSuccess: 'Project created',
    close: 'Close',
    updateSuccess: 'Deal updated',
    markWonSuccess: 'Deal marked as Won',
    wonFollowUpTaskTitle: 'Schedule kickoff call',
    wonFollowUpTaskCreated: 'Follow-up task created: Schedule kickoff call',
  },
}
