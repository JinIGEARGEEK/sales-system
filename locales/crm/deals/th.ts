export default {
  index: {
    pageTitle: 'Deals Pipeline',
    heading: 'Deals Pipeline',
    addDeal: 'เพิ่ม Deal',
    exportCsv: 'ส่งออก CSV',
    manageStages: 'จัดการขั้นตอนไปป์ไลน์',
    dealMovedTo: 'ย้าย Deal ไปยัง {stage} แล้ว',
    searchPlaceholder: 'ค้นหาด้วยชื่อ Deal หรือบริษัท...',
    assigneePlaceholder: 'ผู้รับผิดชอบ',
    disqualifiedLeadBadge: 'Lead ที่ถูกตัดสิทธิ์',
    leadConvertedToDeal: 'แปลง Lead เป็น Deal สำเร็จ',
    leadStatusUpdated: 'ย้าย Lead ไปยัง {status} แล้ว',
    viewKanban: 'Kanban',
    viewList: 'รายการ',
    entityLabel: 'Deal',
    loadMoreDeals: 'โหลดเพิ่มเติม (อีก {count} รายการ)',
  },
  table: {
    searchPlaceholder: 'ค้นหาด้วยชื่อ Deal หรือบริษัท...',
    columns: {
      title: 'ชื่อ Deal',
      company: 'บริษัท',
      value: 'มูลค่า',
      stage: 'ขั้นตอน',
      assignedTo: 'ผู้รับผิดชอบ',
      created: 'สร้างเมื่อ',
      action: 'การดำเนินการ',
    },
    actions: {
      viewDetail: 'ดูรายละเอียด',
      edit: 'แก้ไข',
      delete: 'ลบ',
    },
    deleteSuccess: 'ลบ Deal สำเร็จ',
  },
  create: {
    pageTitle: 'เพิ่ม Deal',
    heading: 'เพิ่ม Deal',
    subheading: 'สร้างโอกาสในการขายใหม่',
    dealTitle: 'ชื่อ Deal',
    dealTitlePlaceholder: 'กรอกชื่อ Deal',
    company: 'บริษัท',
    companyPlaceholder: 'ค้นหาหรือพิมพ์ชื่อบริษัทใหม่',
    primaryContact: 'ผู้ติดต่อหลัก',
    primaryContactPlaceholder: 'เลือกผู้ติดต่อ',
    dealValue: 'มูลค่า Deal (บาท)',
    dealValuePlaceholder: 'กรอกมูลค่า Deal',
    stage: 'ขั้นตอน',
    stagePlaceholder: 'เลือกขั้นตอน',
    forecastCategory: 'หมวดหมู่การคาดการณ์',
    expectedCloseDate: 'วันที่คาดว่าจะปิด Deal',
    businessUnit: 'หน่วยธุรกิจ',
    businessUnitPlaceholder: 'เลือกหน่วยธุรกิจ (ไม่บังคับ)',
    project: 'Project',
    product: 'Product',
    businessUnitItemPlaceholder: 'เลือกรายการ',
    createDeal: 'บันทึก Deal',
    cancel: 'ยกเลิก',
    createSuccess: 'สร้าง Deal สำเร็จ',
    duplicateWarningTitle: 'อาจมี Deal นี้อยู่แล้ว',
    duplicateWarningBody: 'บริษัทนี้มี Deal ที่เปิดอยู่แล้ว:',
  },
  detail: {
    pageTitle: 'รายละเอียด Deal',
    markWon: 'ปิดสำเร็จ',
    tabs: {
      overview: 'ภาพรวม',
      quotes: 'ใบเสนอราคา',
      contracts: 'สัญญา',
      payments: 'การชำระเงิน',
      tasks: 'งานติดตาม',
      activity: 'กิจกรรม',
      attachments: 'ไฟล์แนบ',
    },
    dealTitle: 'ชื่อ Deal',
    dealValue: 'มูลค่า Deal (บาท)',
    stage: 'ขั้นตอน',
    probability: 'โอกาสในการปิดสำเร็จ (%)',
    forecastCategory: 'หมวดหมู่การคาดการณ์',
    lostReason: 'เหตุผลที่ Deal เสีย',
    lostReasonPlaceholder: 'เลือกเหตุผล',
    contractRequiredWarning: 'ต้องมีสัญญาที่เซ็นแล้วก่อนจึงจะปิด Deal นี้เป็นสำเร็จได้',
    contractRequiredToast: 'กรุณาเพิ่มสัญญาที่เซ็นแล้วก่อนปิด Deal นี้เป็นสำเร็จ',
    expectedCloseDate: 'วันที่คาดว่าจะปิด Deal',
    businessUnit: 'หน่วยธุรกิจ',
    businessUnitPlaceholder: 'เลือกหน่วยธุรกิจ (ไม่บังคับ)',
    businessUnitItemPlaceholder: 'เลือกรายการ',
    product: 'Product',
    saveChanges: 'บันทึกการเปลี่ยนแปลง',
    linkedRecords: 'ข้อมูลที่เชื่อมโยง',
    company: 'บริษัท',
    contact: 'ผู้ติดต่อ',
    project: 'Project',
    projectNotCreatedYet: 'ยังไม่ได้สร้าง',
    ownerHistory: 'ประวัติการเปลี่ยนผู้รับผิดชอบ',
    ownerHistoryReassigned: 'เปลี่ยนจาก {from} เป็น {to}',
    ownerHistoryAssigned: 'มอบหมายให้ {to}',
    ownerHistoryBy: 'โดย {actor}',
    noOwnerHistory: 'ยังไม่มีการเปลี่ยนผู้รับผิดชอบ',
    quotesTitle: 'ใบเสนอราคา',
    createQuote: 'สร้างใบเสนอราคา',
    createQuoteSuccess: 'สร้างใบเสนอราคาสำเร็จ',
    noQuotes: 'ยังไม่มีใบเสนอราคาสำหรับ Deal นี้',
    validUntil: 'ใช้ได้ถึง {date}',
    uploadQuotation: 'อัปโหลดใบเสนอราคา (PDF)',
    invalidFileType: 'กรุณาอัปโหลดไฟล์ PDF เท่านั้น',
    fileTooLarge: 'ไฟล์มีขนาดใหญ่เกินไป (สูงสุด 10 MB)',
    uploadSuccess: 'อัปโหลดใบเสนอราคาสำเร็จ',
    uploadedOn: 'อัปโหลดเมื่อ {date}',
    viewPdf: 'ดูไฟล์ PDF',
    editQuote: 'แก้ไขใบเสนอราคา',
    downloadPdf: 'ดาวน์โหลดเป็น PDF',
    removeQuotation: 'ลบใบเสนอราคา',
    removeQuotationConfirmBody: 'ลบ "{name}" หรือไม่? การดำเนินการนี้ไม่สามารถย้อนกลับได้',
    removeQuotationSuccess: 'ลบใบเสนอราคาสำเร็จ',
    updateQuoteStatusSuccess: 'อัปเดตสถานะใบเสนอราคาสำเร็จ',
    paymentsTitle: 'การชำระเงิน',
    addPayment: 'เพิ่มการชำระเงิน',
    noPayments: 'ยังไม่มีการชำระเงินสำหรับ Deal นี้',
    totalPaid: 'ยอดชำระแล้ว',
    remainingBalance: 'ยอดคงเหลือ',
    fullyPaid: 'ชำระครบแล้ว',
    columnDate: 'วันที่',
    columnAmount: 'จำนวนเงิน',
    columnMethod: 'ช่องทาง',
    columnNote: 'หมายเหตุ',
    removePayment: 'ลบการชำระเงิน',
    removePaymentConfirmBody: 'ลบรายการชำระเงิน {amount} เมื่อ {date} หรือไม่? การดำเนินการนี้ไม่สามารถย้อนกลับได้',
    addPaymentSuccess: 'บันทึกการชำระเงินสำเร็จ',
    removePaymentSuccess: 'ลบการชำระเงินสำเร็จ',
    paymentScheduleTitle: 'กำหนดการชำระเงิน',
    addInstallment: 'เพิ่มงวดชำระ',
    noInstallments: 'ยังไม่มีกำหนดการชำระเงินสำหรับ Deal นี้',
    columnDueDate: 'วันครบกำหนด',
    columnStatus: 'สถานะ',
    installmentStatus: {
      paid: 'ชำระแล้ว',
      partial: 'ชำระบางส่วน',
      overdue: 'เกินกำหนด',
      upcoming: 'ยังไม่ถึงกำหนด',
    },
    removeInstallment: 'ลบงวดชำระ',
    removeInstallmentConfirmBody: 'ลบงวดชำระ {amount} ที่ครบกำหนด {date} ใช่หรือไม่? ไม่สามารถย้อนกลับได้',
    addInstallmentSuccess: 'เพิ่มงวดชำระสำเร็จ',
    removeInstallmentSuccess: 'ลบงวดชำระสำเร็จ',
    generateSchedule: 'สร้างกำหนดการชำระ',
    generateScheduleSuccess: 'สร้างงวดชำระ {count} งวดสำเร็จ',
    tasksTitle: 'งานติดตาม',
    addTask: 'เพิ่มงานติดตาม',
    addTaskSuccess: 'เพิ่มงานติดตามสำเร็จ',
    editTaskSuccess: 'แก้ไขงานติดตามสำเร็จ',
    activityTitle: 'กิจกรรม',
    addActivity: 'เพิ่มกิจกรรม',
    addActivitySuccess: 'บันทึกกิจกรรมสำเร็จ',
    stageHistoryTitle: 'ประวัติการเปลี่ยนสถานะ',
    stageHistoryBy: 'โดย {actor}',
    attachmentsTitle: 'ไฟล์แนบ',
    addAttachment: 'เพิ่มไฟล์แนบ',
    addAttachmentSuccess: 'เพิ่มไฟล์แนบสำเร็จ',
    removeAttachmentSuccess: 'ลบไฟล์แนบสำเร็จ',
    dealNotFound: 'ไม่พบ Deal',
    createProjectModalTitle: 'สร้าง Project จาก Deal นี้หรือไม่?',
    createProjectModalBody: 'การดำเนินการนี้จะสร้าง Project ใหม่ให้บริษัทนี้ และ Link กลับไปยัง Deal นี้',
    createProjectSuccess: 'สร้าง Project สำเร็จ',
    close: 'ปิด',
    updateSuccess: 'แก้ไข Deal สำเร็จ',
    markWonSuccess: 'ปิด Deal สำเร็จแล้ว',
    wonFollowUpTaskTitle: 'นัดหมาย Kickoff Call',
    wonFollowUpTaskCreated: 'สร้างงานติดตามแล้ว: นัดหมาย Kickoff Call',
  },
}
