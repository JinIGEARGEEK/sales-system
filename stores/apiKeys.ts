...
//
// No local `items` cache: nothing else in the app reads an API key by id
// (unlike Users, whose `items` also backs "created/updated by" name lookups
// elsewhere) — the Admin list page is this store's only consumer, and reads
// entirely through `fetchList` + `useServerListPage`, refetching after
// add/revoke rather than hand-mutating a cached array.
...
export const useApiKeysStore = defineStore('apiKeys', {
  actions: {
    // GET /admin/api-keys is genuinely paginated server-side (§8.7e) — this
    // pairs with useServerListPage the same way stores/users.ts's own
    // fetchList does, so an org with more than one page of keys can still
    // reach the rest instead of them silently being capped out of the list.
    async fetchList (params?: Record<string, unknown>) {
      const { $api } = useNuxtApp()
      const response = await $api.get<ApiResponse<APIKey[]>>('/admin/api-keys', { params })
      return {
    ...
    async add (payload: { name: string, owner_user_id: number }): Promise<{ apiKey: APIKey, key: string }> {
      ...
      return { apiKey: parseDates(response.data.data.api_key), key: response.data.data.key }
    },
